...
- **Luzes de status + sininho vermelho (2026-07-07):** faixa `#admin-uptime-lights` acima do Monitor CPU (aba master) com um ponto+luz por alvo (verde=no ar, vermelho pulsa=fora do ar), via `GET /admin/infra/uptime-monitor/lights` (`getUptimeLights` — check rápido 2 tentativas, cache 45s). Polling acoplado ao ciclo de alerta CPU (60s). Sininho fica vermelho (`has-system-down`) + item "⚠️ Sistema fora do ar" **apenas** para `walkup@walkuptec.com.br` quando alguma luz vermelha. Arquivos: `uptime-monitor.service.ts`, `waba-admin.routes.ts`, `index.html`. Log `doc/LOG-2026-07-07__154000__ui-luzes-status-monitor-cpu-sininho-walkup.md`. Em `v02`, falta deploy.
- **Monitor de uptime unificado (2026-07-07):** novo `src/monitoring/uptime-monitor.service.ts` roda a cada 15min (setInterval, prod/v01) e checa `draxsistemas.com.br`, `bet.waba.info`, `wabadisparos.com.br`, `waba.draxsistemas.com.br/health` (fetch, 3 tentativas, online=200–399) + Asaas (`evaluateAsaasIntegrationHealth`). Falha → WhatsApp `5551999666841` (instância por telefone `51981077770`→fallback `51997462102`, validada via `fetchEvoInstanceLiveState`) + e-mail `walkup@walkuptec.com.br`. Dedup em `data/uptime-monitor-state.json` (alerta na queda, reenvio a cada `REALERT_MINUTES`=60, aviso de recuperação). Rotas master: `GET/POST /admin/infra/uptime-monitor/{status,run,test-alert}`. Env `WABA_UPTIME_MONITOR_*`. Substitui monitor antigo (só logava). Build OK; falta deploy. Log `doc/LOG-2026-07-07__150500__monitor-uptime-15min-whatsapp-email.md`. Palavras-chave: uptime monitor, 15min, alerta whatsapp/email, asaas webhook.
