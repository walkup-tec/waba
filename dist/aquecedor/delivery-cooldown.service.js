...
function enqueueWrite(store) {
    ...
    writeTimer = setTimeout(() => {
        ...
        void promises_1.default
            .mkdir(path_1.default.dirname(STORE_FILE), { recursive: true })
            .then(() => promises_1.default.writeFile(STORE_FILE, JSON.stringify(store, null, 2), "utf8"))
...
async function recordDirectedDeliveryFailure(input) {
    ...
    const untilMs = nowMs + Math.max(60000, input.cooldownMs ?? DEFAULT_COOLDOWN_MS);
    store.directed[key] = {
        untilMs,
