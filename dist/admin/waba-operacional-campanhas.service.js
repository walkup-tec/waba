...
class WabaOperacionalCampanhasService {
    ...
    assertCanMutateCampaigns(staff) {
        if (staff.role === "indicador") {
    ...
    toListItem(intake, staff, broadcastProgress) {
        ...
        return {
            ...
            canBmInoperante: !readOnly && status === "generated" && !String(intake.bmInoperanteRegisteredAt || "").trim(),
    ...
    listTransferOperacionais(campaignId, staff) {
        if (staff.role !== "master" && !(0, waba_auth_service_1.isWabaMasterEmail)(staff.email)) {
