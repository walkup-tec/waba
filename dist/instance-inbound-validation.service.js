...
var instance_inbound_validation_service_exports = {};
__export(instance_inbound_validation_service_exports, {
  INBOUND_VALIDATION_KEYWORD: () => INBOUND_VALIDATION_KEYWORD,
  confirmUserSentInbound: () => confirmUserSentInbound,
  getInboundValidationStatus: () => getInboundValidationStatus,
  handleInboundValidationWebhook: () => handleInboundValidationWebhook,
  pruneInboundValidations: () => pruneInboundValidations,
  refreshInboundValidation: () => refreshInboundValidation,
  setInboundValidationFinishedHandler: () => setInboundValidationFinishedHandler,
  startInboundValidation: () => startInboundValidation
});
module.exports = __toCommonJS(instance_inbound_validation_service_exports);
var import_crypto = __toESM(require("crypto"));
var import_evo_http = require("./evo-http.client");
var import_evo_connection_state = require("./instances/evo-connection-state.service");
var import_evo_instance_phone = require("./instances/evo-instance-phone.service");
var import_waba_public_base_url = require("./lib/waba-public-base-url");
...
const EVO_SEND_TEXT_URL_TEMPLATE = String(process.env.EVO_SEND_TEXT_URL_TEMPLATE || "").trim() || `${EVO_API_BASE}/message/sendText/{instance}`;
const EVO_SEND_TEXT_V1 = process.env.EVO_SEND_TEXT_V1 === "1" || process.env.EVO_SEND_TEXT_V1 === "true";
const INBOUND_VALIDATION_KEYWORD = String(process.env.INBOUND_VALIDATION_KEYWORD || "CONFIRMAR").trim() || "CONFIRMAR";
const VALIDATION_TIMEOUT_MS = Math.max(
  12e4,
  Math.min(9e5, Number(process.env.INBOUND_VALIDATION_TIMEOUT_MS || 6e5) || 6e5)
);
const VALIDATION_WORKER_MS = Math.max(
  1e3,
  Math.min(1e4, Number(process.env.INBOUND_VALIDATION_WORKER_MS || 2e3) || 2e3)
);
const VALIDATION_POLL_CACHE_MS = Math.max(
  1e3,
  Math.min(1e4, Number(process.env.INBOUND_VALIDATION_POLL_CACHE_MS || 2e3) || 2e3)
);
const FIND_MESSAGES_TIMEOUT_MS = Math.max(
  3e3,
  Math.min(2e4, Number(process.env.INBOUND_VALIDATION_FIND_MSG_TIMEOUT_MS || 8e3) || 8e3)
);
...
const REPLY_DELAY_MS = Math.max(
  500,
  Math.min(3e4, Number(process.env.INBOUND_VALIDATION_REPLY_DELAY_MS || 1e3) || 1e3)
);
const validations = /* @__PURE__ */ new Map();
const activeValidationByInstance = /* @__PURE__ */ new Map();
const recentReplyByConversation = /* @__PURE__ */ new Map();
const REPLY_DEDUPE_MS = 15 * 60 * 1e3;
const replyInFlight = /* @__PURE__ */ new Set();
...
function normalizeWhatsAppNumber(num) {
  return (0, import_evo_instance_phone.normalizeEvoWhatsAppNumber)(num);
}
function formatPhoneHint(num) {
  const digits = (0, import_evo_instance_phone.canonicalizeBrazilWhatsAppNumber)(num) || normalizeWhatsAppNumber(num);
  if (!digits) return "";
...
function resolvePublicWebhookBase() {
  return String((0, import_waba_public_base_url.resolveWabaPublicBaseUrl)() || "").trim().replace(/\/+$/, "");
}
...
async function callEvo(url, method, body, options) {
  ...
  const result = await (0, import_evo_http.evoHttpRequest)(url, method, {
    apiKey: EVO_API_KEY,
    body,
    timeoutMs: options?.timeoutMs ?? (isSendText ? (0, import_evo_http.defaultEvoSendTextTimeoutMs)() : 15e3),
...
function buildTemplateUrl(template, instanceName) {
  return template.replace("{instance}", encodeURIComponent(instanceName)).replace("{name}", encodeURIComponent(instanceName));
}
...
function buildSendNumberCandidates(referenceJid, referenceNumber, preferredExact) {
  const out = [];
  ...
  const fromJidUser = jid.includes("@") ? normalizeWhatsAppNumber(jid.split("@")[0] || "") : normalizeWhatsAppNumber(jid);
  const seedDigits = [fromNumber, fromJidUser].filter(Boolean);
  for (const seed of seedDigits) {
    for (const variant of (0, import_evo_instance_phone.expandBrazilWhatsAppNumberVariants)(seed)) {
...
function normalizeKeywordText(text) {
  return String(text || "").trim().toLowerCase().normalize("NFD").replace(/\p{M}/gu, "").replace(/[^\p{L}\p{N}]/gu, "");
}
function keywordMatchNeedles(keyword) {
  const primary = normalizeKeywordText(keyword);
  if (!primary) return [];
  const needles = /* @__PURE__ */ new Set([primary]);
...
function extractMessageTimestampMs(node) {
  ...
  for (const raw of candidates) {
    ...
    return n < 1e12 ? Math.round(n * 1e3) : Math.round(n);
  }
  return null;
}
function isSendFailureTechnical(detail, httpStatus) {
  ...
  if (d.includes("timeout") || d.includes("socket hang up") || d.includes("econnreset") || d.includes("network") || d.includes("http 0")) {
    return true;
  }
  return false;
}
...
function tryFinalize(record) {
  ...
  record.phase = record.receiveTest.success === true && record.sendTest.success === true ? "validated" : "failed";
...
async function finalizeExpiredAsync(record) {
  ...
  if (record.receiveTest.success === null) {
    try {
      ...
      if (hit) {
        ...
        await new Promise((r) => setTimeout(r, REPLY_DELAY_MS + 1e3));
        await runValidationFollowUp(record);
      }
    } catch {
    }
  ...
  if (record.finished) return;
  const phoneLabel = formatPhoneHint(record.instanceNumber) || "n\xFAmero integrado";
  if (record.receiveTest.success === null) {
    record.receiveTest = {
      success: false,
      detail: `Tempo esgotado sem receber "${record.keyword}" de outro WhatsApp para ${phoneLabel}. Abra o chat com esse n\xFAmero (n\xE3o o celular que escaneou o QR) e envie s\xF3 a palavra ${record.keyword}.`
    };
  ...
  if (record.sendTest.success === null) {
    record.sendTest = {
      success: false,
      detail: record.receiveTest.success ? "Tempo esgotado sem confirmar resposta na conversa." : "Resposta n\xE3o testada \u2014 recep\xE7\xE3o n\xE3o confirmada."
    };
  }
  tryFinalize(record);
}
async function readInstanceWebhook(instanceName) {
  ...
  for (const url of findUrls) {
    const result = await callEvo(url, "GET", void 0, { timeoutMs: 8e3 });
...
async function ensureInstanceWebhook(instanceName) {
  ...
  if (existing.enabled && existing.url && (webhookUrl ? existing.url === webhookUrl : existing.url.includes("/webhooks/evolution"))) {
    return true;
  }
  ...
  for (const setUrl of setUrls) {
    const result = await callEvo(setUrl, "POST", body, { timeoutMs: 1e4 });
...
const INBOUND_CLOCK_SKEW_MS = Math.max(
  0,
  Math.min(5e3, Number(process.env.INBOUND_VALIDATION_CLOCK_SKEW_MS || 2e3) || 2e3)
);
...
function extractRemoteJid(node) {
  ...
  const candidates = [
    ...
    key?.participant,
    node.participant
  ].map((value) => String(value || "").trim()).filter((s) => s && !s.includes("@g.us"));
...
async function findInboundViaApiFast(instanceName, keyword, searchOpts) {
  ...
  const probes = urls.flatMap(
    (url) => bodies.map(async (body) => {
...
async function resolveInboundHit(record, options = false) {
...
async function captureKeywordHighWaterMark(instanceName, keyword) {
  let maxTs = 0;
  const collectMax = (payload) => {
    const hits = [];
...
async function findReplyInChat(instanceName, referenceJid, replyMarker, referenceNumber, minTimestampMs) {
  const marker = String(replyMarker || "").trim().toLowerCase();
  if (!marker) return false;
  const remoteCandidates = /* @__PURE__ */ new Set();
  const jid = String(referenceJid || "").trim();
...
function markInboundReceived(record, hit, via) {
  ...
  const phoneJid = phoneDigits.length >= 12 ? `${phoneDigits}@s.whatsapp.net` : "";
  const inboundJid = String(hit.remoteJid || "").trim();
  record.referenceNumber = phoneDigits || hit.referenceNumber;
  record.inboundChatJid = phoneJid || (isPhoneWhatsAppJid(inboundJid) && !isLidJid(inboundJid) ? inboundJid : "") || inboundJid || null;
  record.referenceJid = phoneJid || (isPhoneWhatsAppJid(inboundJid) && !isLidJid(inboundJid) ? inboundJid : inboundJid) || null;
...
async function runValidationFollowUp(record) {
  ...
  if (record.sendAttempted && record.sendHttpOk && record.sendTest.success !== true && (record.inboundChatJid || record.referenceJid)) {
    ...
    if (found) {
      record.sendTest = {
        success: true,
        detail: "Resposta confirmada no hist\xF3rico da conversa."
      };
      tryFinalize(record);
    }
  }
...
async function sendContextualReply(record) {
  ...
  if (!open) {
    ...
    record.sendDetail = "Inst\xE2ncia n\xE3o est\xE1 open para enviar a resposta.";
    record.sendTest = {
      success: false,
      detail: "A inst\xE2ncia n\xE3o ficou conectada a tempo para enviar \xABValida\xE7\xE3o WABA conclu\xEDda\xBB. Escaneie o QR de novo ou use Atualizar."
    };
    tryFinalize(record);
    return;
  }
  ...
  if (convKey) {
    ...
    if (lastSentAt != null && Date.now() - lastSentAt < REPLY_DEDUPE_MS) {
      ...
      const found = proofJid && await findReplyInChat(
        record.instanceName,
        proofJid,
        record.replyMarker,
        record.referenceNumber,
      ...
      if (found) {
        ...
        record.sendTest = {
          success: true,
          detail: "Resposta j\xE1 enviada nesta conversa (valida\xE7\xE3o \xFAnica)."
        };
        tryFinalize(record);
        return;
      }
  ...
  record.sendAttempted = true;
  record.sendAttemptedAtMs = Date.now();
  const text = `Valida\xE7\xE3o WABA conclu\xEDda. ${record.replyMarker}`;
  const sendUrl = buildTemplateUrl(EVO_SEND_TEXT_URL_TEMPLATE, record.instanceName);
  const preferred = record.inboundChatJid || record.referenceJid || record.referenceNumber || "";
  ...
  if (!candidates.length || candidates.every((c) => isLidJid(c))) {
    ...
    record.sendHttpOk = false;
    record.sendDetail = "Destino da resposta n\xE3o identificado.";
    record.sendTest = {
      success: false,
      detail: "N\xE3o foi poss\xEDvel identificar o n\xFAmero do outro WhatsApp para responder (JID @lid sem telefone)."
    };
    tryFinalize(record);
    return;
  }
  let lastDetail = "";
  let anyHttpOk = false;
  const replyMinTs = (record.sendAttemptedAtMs || record.validationStartedAtMs) - INBOUND_CLOCK_SKEW_MS;
  const proofChatJid = record.inboundChatJid || record.referenceJid || preferred;
  try {
    for (const numero of candidates) {
      if (record.cancelled || record.finished) return;
      const sendBody = EVO_SEND_TEXT_V1 ? { number: numero, textMessage: { text } } : { number: numero, text, textMessage: { text } };
      const result = await callEvo(sendUrl, "POST", sendBody);
      if (!result.ok) {
        const detail = result.json?.message || result.json?.error || result.body.slice(0, 180) || `HTTP ${result.status}`;
      ...
      record.sendTest = {
        success: null,
        detail: "Resposta enviada; confirmando no hist\xF3rico da conversa\u2026"
      };
    ...
    if (anyHttpOk) {
      ...
      record.sendTest = {
        success: null,
        detail: "A API aceitou o envio, mas a mensagem ainda n\xE3o apareceu no chat do CONFIRMAR. Aguarde ou reenvie CONFIRMAR."
      };
      return;
    }
    record.sendHttpOk = false;
    ...
    record.sendTest = {
      success: false,
      detail: restricted ? `O sistema WABA - Drax recusou a resposta: ${record.sendDetail}` : `Falha ao enviar resposta autom\xE1tica: ${record.sendDetail}`
    };
    tryFinalize(record);
  } finally {
    if (convKey) replyInFlight.delete(convKey);
...
async function ensureValidationInstanceOpen(record) {
  const maxWaitMs = Math.max(
    5e3,
    Math.min(3e4, Number(process.env.INBOUND_VALIDATION_OPEN_WAIT_MS || 12e3) || 12e3)
  );
  const wait = await (0, import_evo_connection_state.waitForEvoInstanceLiveOpen)(record.instanceName, {
    maxWaitMs,
    pollMs: 400
  });
  return wait.open;
...
async function processValidationRecordInWorker(record) {
  ...
  if (record.receiveTest.success === true && record.inboundReceivedAt && Date.now() >= record.inboundReceivedAt + REPLY_DELAY_MS && !record.sendAttempted) {
    await sendContextualReply(record);
  }
  if (record.sendAttempted && record.sendHttpOk && record.sendTest.success !== true && (record.inboundChatJid || record.referenceJid)) {
    const found = await findReplyInChat(
...
async function bootstrapInboundValidation(record) {
  ...
  if (!open) {
    record.receiveTest = {
      success: false,
      detail: `Inst\xE2ncia n\xE3o conectou no sistema WABA - Drax a tempo. Escaneie o QR novamente.`
    };
    record.sendTest = {
      success: false,
      detail: "Valida\xE7\xE3o cancelada \u2014 inst\xE2ncia desconectada."
    };
    record.phase = "failed";
    record.finished = true;
    record.finishedAt = (/* @__PURE__ */ new Date()).toISOString();
...
function findInboundInWebhookChunk(chunk, keyword, record) {
...
function handleInboundValidationWebhook(body) {
  ...
  const candidates = active ? [active] : [...validations.values()].filter((record) => isRecordActive(record));
...
function isEvolutionMessageUpsertEvent(eventRaw) {
  const event = String(eventRaw || "").trim().toUpperCase().replace(/\./g, "_");
...
async function startInboundValidation(input) {
  ...
  if (!instanceName) {
    return { error: "Nome da inst\xE2ncia \xE9 obrigat\xF3rio." };
  }
  const numberHint = normalizeWhatsAppNumber(String(input.instanceNumberHint || "").trim());
  const resolvedNumber = await (0, import_evo_instance_phone.resolveEvoInstancePhone)(instanceName, { hint: numberHint });
  ...
  const validationId = import_crypto.default.randomUUID();
  ...
  const receiveWaitDetail = phoneLabel ? `Envie "${keyword}" de outro WhatsApp para ${phoneLabel}. O sistema detecta automaticamente.` : `Envie "${keyword}" de outro WhatsApp (n\xE3o o celular do QR). O sistema detecta automaticamente.`;
...
async function confirmUserSentInbound(validationId) {
  ...
  if (!id) {
    return { ok: false, found: false, error: "validationId \xE9 obrigat\xF3rio.", status: null };
  }
  const record = validations.get(id);
  if (!record) {
    return {
      ok: false,
      found: false,
      error: "Valida\xE7\xE3o n\xE3o encontrada ou expirada.",
      status: null
    };
  }
  if (record.finished) {
    const status2 = getInboundValidationStatus(id);
    return {
      ok: true,
      found: status2?.receiveTest?.success === true,
      status: status2
    };
  }
