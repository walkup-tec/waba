...
exports.hasCasaDosDadosApiKey = hasCasaDosDadosApiKey;
exports.buildCasaDosDadosPesquisaBody = buildCasaDosDadosPesquisaBody;
exports.fetchCasaDosDadosLeadsViaApi = fetchCasaDosDadosLeadsViaApi;
const waba_leads_cnpj_repository_1 = require("./waba-leads-cnpj.repository");
const API_BASE = String(process.env.CASADOSDADOS_API_BASE || "https://api.casadosdados.com.br").replace(/\/$/, "");
function hasCasaDosDadosApiKey() {
    return Boolean(String(process.env.CASADOSDADOS_API_KEY || "").trim());
}
function readApiKey() {
    const key = String(process.env.CASADOSDADOS_API_KEY || "").trim();
    if (!key) {
        throw new Error("CASADOSDADOS_API_KEY ausente. Gere em https://portal.casadosdados.com.br/plataforma/api/chave e configure no .env.v02.");
    }
    return key;
}
...
function porteCodigo(value) {
    const raw = String(value || "").trim().toLowerCase();
    if (!raw)
        return undefined;
    if (/^01$|micro/.test(raw))
        return "01";
    if (/^03$|pequeno|epp/.test(raw))
        return "03";
    if (/^05$|demais|grande|medio|médio/.test(raw))
        return "05";
    return undefined;
}
/** Monta o body da Pesquisa Avançada v5 a partir dos filtros da UI. */
function buildCasaDosDadosPesquisaBody(filters, pagina, limite) {
    const body = {
        limite,
        pagina,
    };
    const cnpj = (0, waba_leads_cnpj_repository_1.normalizeCnpjDigits)(filters.cnpj || "");
    if (cnpj.length === 14)
        body.cnpj = [cnpj];
    ...
    if (natureza)
        body.codigo_natureza_juridica = [natureza];
    const situacoes = (filters.situacaoCadastral || [])
        .map((s) => String(s || "").trim().toUpperCase())
        .filter(Boolean);
    if (situacoes.length)
        body.situacao_cadastral = situacoes;
    if (filters.somenteMatriz)
        body.matriz_filial = "MATRIZ";
    else if (filters.somenteFilial)
        body.matriz_filial = "FILIAL";
    const raiz = String(filters.cnpjRaiz || "").replace(/\D/g, "").slice(0, 8);
    if (raiz.length === 8)
        body.cnpj_raiz = [raiz];
    if (filters.cep)
        body.cep = [String(filters.cep).replace(/\D/g, "")];
    if (filters.estadoUf)
        body.uf = [String(filters.estadoUf).trim().toLowerCase()];
    if (filters.municipio)
        body.municipio = [String(filters.municipio).trim().toLowerCase()];
    if (filters.bairro)
        body.bairro = [String(filters.bairro).trim().toLowerCase()];
    if (filters.ddd)
        body.ddd = [String(filters.ddd).replace(/\D/g, "")];
    if (filters.telefone)
        body.telefone = [String(filters.telefone).replace(/\D/g, "")];
    ...
    if (Object.keys(mais).length)
        body.mais_filtros = mais;
    return body;
}
function mapApiItemToLead(item) {
    const cnpj = (0, waba_leads_cnpj_repository_1.normalizeCnpjDigits)(item.cnpj || item.CNPJ);
    if (cnpj.length !== 14)
        return null;
...
async function postPesquisa(apiKey, body, tipoResultado) {
    ...
    try {
        ...
        if (!res.ok) {
            const msg = String(json.mensagem || json.message || json.error || text || "").slice(0, 300) ||
                `HTTP ${res.status}`;
            if (res.status === 401) {
        ...
        const cnpjs = Array.isArray(json.cnpjs) ? json.cnpjs : [];
 ...
 */
async function fetchCasaDosDadosLeadsViaApi(filters, onProgress) {
    const apiKey = readApiKey();
    const pageSize = Math.min(1000, Math.max(1, Math.round(Number(process.env.CASADOSDADOS_PAGE_SIZE || 100) || 100)));
    ...
    const pagesToFetch = maxPagesCap > 0
        ? Math.min(maxPagesCap, Math.max(1, totalPagesAvailable))
        : Math.max(1, totalPagesAvailable);
    onProgress?.(`Pesquisa retornou ${total.toLocaleString("pt-BR")} empresas. Coletando página 1/${pagesToFetch} (${pageSize}/página · só CNPJ + Razão Social)…`);
    ...
    for (let pagina = 2; pagina <= pagesToFetch; pagina += 1) {
        onProgress?.(`Pesquisa retornou ${total.toLocaleString("pt-BR")} empresas. Coletando página ${pagina}/${pagesToFetch} (${collected.size.toLocaleString("pt-BR")} CNPJs)…`);
        const page = await postPesquisa(apiKey, buildCasaDosDadosPesquisaBody(filters, pagina, pageSize), tipo);
        if (!page.cnpjs.length)
            break;
        ingest(page.cnpjs);
        if (collected.size >= total && total > 0)
            break;
    }
    if (!collected.size) {
    ...
    onProgress?.(`Coletados ${collected.size.toLocaleString("pt-BR")} de ${total.toLocaleString("pt-BR")} (CNPJ + Razão Social · páginas ${pagesToFetch}/${totalPagesAvailable}). Enriquecimento na sequência.`);
