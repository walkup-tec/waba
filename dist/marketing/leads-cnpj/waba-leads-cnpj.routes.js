...
exports.registerWabaLeadsCnpjRoutes = void 0;
const waba_request_auth_1 = require("../../auth/waba-request-auth");
const waba_leads_cnpj_service_1 = require("./waba-leads-cnpj.service");
const service = new waba_leads_cnpj_service_1.WabaLeadsCnpjService();
...
const registerWabaLeadsCnpjRoutes = (app) => {
    ...
    app.post("/admin/marketing/leads-cnpj/:id/downloaded", (req, res) => {
        ...
        try {
            ...
            if (!item)
                return res.status(404).json({ error: "Lista não encontrada ou sem Excel." });
            return res.status(200).json({ item });
        }
        catch (error) {
            return res.status(500).json({
    ...
    app.post("/admin/marketing/leads-cnpj/:id/resume-scrape", (req, res) => {
        ...
        try {
            ...
            return res.status(400).json({
                error: error instanceof Error ? error.message : "Não foi possível retomar a raspagem do portal.",
            });
        }
    });
...
exports.registerWabaLeadsCnpjRoutes = registerWabaLeadsCnpjRoutes;
