...
function isKeepaliveProgressMessage(message) {
...
async function isCloudflareInterstitial(page) {
    ...
    if (isPortalChallengeHint({ title, url }))
        return true;
    const hint = await withNodeTimeout(page
        .evaluate(() => {
        ...
        .catch(() => ({ text: "", hasWidget: false })), 2500, null);
    if (hint === null)
        return "hung";
    if (hint.hasWidget)
        return true;
...
page, options) {
    ...
    if (blocked === "hung") {
        throw new LeadsScrapeError("CDP_PROBE_TIMEOUT", "new-browser", `Probe anti-bot travou (${stage}) — CDP/title sem resposta.`);
    }
    if (!blocked)
        return;
    ...
    while (Date.now() - started < timeoutMs) {
        ...
        if (still === "hung") {
            throw new LeadsScrapeError("CDP_PROBE_TIMEOUT", "new-browser", `Probe anti-bot travou no meio do desafio (${stage}).`);
        }
        if (!still) {
            await sleepNode(600);
            const again = await isCloudflareInterstitial(page);
            if (again === "hung") {
                throw new LeadsScrapeError("CDP_PROBE_TIMEOUT", "new-browser", `Probe anti-bot travou após aparente liberação (${stage}).`);
            }
            if (!again)
                return;
        }
...
page, url, options) {
    ...
    for (let attempt = 1; attempt <= 3; attempt += 1) {
        try {
            ...
            if (!alive)
                throw error;
            await withNodeTimeout(page.waitForLoadState("domcontentloaded").then(() => true), 2500, false);
...
page, email, password) {
    ...
    try {
        ...
        const emailReady = await withNodeTimeout(emailInput.waitFor({ state: "visible", timeout: 12000 }).then(() => true), 14000, false);
        const passwordReady = await withNodeTimeout(passwordInput.waitFor({ state: "visible", timeout: 8000 }).then(() => true), 10000, false);
        const accessReady = await withNodeTimeout(accessBtn.waitFor({ state: "visible", timeout: 8000 }).then(() => true), 10000, false);
        if (!emailReady || !passwordReady || !accessReady) {
            throw new LeadsScrapeError("LOGIN_TIMEOUT", "new-browser", `Formulário de login não respondeu (email=${emailReady} senha=${passwordReady} acessar=${accessReady}).`);
        }
 ...
 */
function resolvePortalResumePage(rawPage, floorPage) {
 ...
 */
class LeadsScrapeError extends Error {
    constructor(code, recovery, message) {
        super(message);
        this.code = code;
...
exports.LeadsScrapeError = LeadsScrapeError;
function isLeadsScrapeError(error) {
    return error instanceof LeadsScrapeError;
}
function isSoftScrapeError(error) {
...
function formatProbeShort(p) {
    ...
    const winLabel = win
        ...
        : "none";
    return (`url=${p.url.replace(/^https?:\/\/[^/]+/, "")} ` +
...
async function dismissBlockingPortalOverlays(page) {
    ...
    await withNodeTimeout(page.keyboard.press("Escape").then(() => undefined), 800, undefined);
    await sleepNode(150);
    await withNodeTimeout(page.evaluate(() => {
        const hasBatch = Array.from(document.querySelectorAll("textarea")).some((el) => /cnpj por linha/i.test(String(el.getAttribute("placeholder") || "")));
        if (!hasBatch)
            return false;
        const btn = Array.from(document.querySelectorAll("button")).find((b) => /^(Cancelar|Fechar)$/i.test(String(b.textContent || "").trim()));
        btn?.click();
        return true;
    }), 2000, false);
    await sleepNode(100);
...
function requiresBrowserRecovery(error) {
    if (error instanceof LeadsScrapeError)
        return error.recovery === "new-browser";
    if (error instanceof RendererUnresponsiveError)
        return true;
    const anyErr = error;
    if (anyErr?.recovery === "new-browser")
        return true;
...
async function findSearchButtonCandidates(page) {
    return page.evaluate(() => {
        ...
        const scoreEl = (el, text, aria, title) => {
            ...
            if (!combined.includes("pesquisar") && !combined.includes("buscar"))
                return null;
            let score = 0;
            if (nt === "pesquisar" || nt === "buscar")
                score += 100;
            if (nt.includes("pesquisar"))
                score += 70;
            if (nt.includes("buscar"))
                score += 50;
            if (na.includes("pesquisar") || na.includes("buscar"))
                score += 60;
            if (nti.includes("pesquisar") || nti.includes("buscar"))
                score += 40;
        ...
        const toCandidate = (el, score, source) => {
            const r = el.getBoundingClientRect();
            const text = el instanceof HTMLInputElement ? clean(el.value) : clean(el.textContent);
            return {
                tag: el.tagName,
        ...
        for (const raw of Array.from(document.querySelectorAll(selector))) {
            if (seen.has(raw) || !(raw instanceof HTMLElement))
                continue;
            seen.add(raw);
            if (!isVisible(raw))
                continue;
            const text = raw instanceof HTMLInputElement ? clean(raw.value) : clean(raw.textContent);
            ...
            if (text.length > 80 &&
                ...
                if (!shortSelf.includes("pesquis") && !shortSelf.includes("busc"))
                    continue;
            }
        ...
        for (const raw of Array.from(root.querySelectorAll("*")).slice(0, 2500)) {
            if (!(raw instanceof HTMLElement))
                continue;
            const own = clean(Array.from(raw.childNodes)
...
async function dumpVisibleActions(page) {
    return page.evaluate(() => {
        const clean = (v) => String(v || "")
            ...
            .trim();
        const root = document.querySelector("main") ?? document.body;
        const elements = Array.from(root.querySelectorAll('button, a, [role="button"], input[type="submit"], input[type="button"], [tabindex]'));
        return elements
            .map((el) => {
            ...
            return {
                tag: el.tagName,
                text: el instanceof HTMLInputElement
                    ? clean(el.value)
...
async function dumpSearchTextNodes(page) {
    return page.evaluate(() => {
        const clean = (v) => String(v || "")
            .replace(/\s+/g, " ")
            .trim();
...
async function captureSearchDiagnostics(page) {
    ...
    console.warn("[Leads PJ] SEARCH_ACTION_DUMP", JSON.stringify({ actions, texts, at: new Date().toISOString() }));
}
...
async function readSearchState(page) {
    ...
    const emptyHint = probe.cnpjNodes === 0 &&
        !probe.pagination &&
        /nenhum resultado|0\s+empresas|não\s+encontr|nao\s+encontr/i.test(sampleTotals);
    const blocked = probe.challengeNodes > 0 ||
        /cloudflare|just a moment|verificação de segurança|checking your browser|um momento/i.test(`${probe.url} ${probe.searchButtonText || ""}`);
    return {
...
async function withNodeTimeout(promise, timeoutMs, fallback) {
    let timer;
    try {
 ...
 */
async function waitForSearchAck(page, before, timeoutMs = 5000) {
    ...
    while (Date.now() < deadline) {
        ...
        const full = await withNodeTimeout(probeSearchState(page), 2000, null);
        if (full)
            return full;
        return {
            ...before,
...
async function clickSearchByMouse(page, candidate) {
    if (!page.mouse?.click)
        return false;
    // Botão fica no rodapé do formulário — fora do viewport → clique fantasma.
    await withNodeTimeout(page.evaluate((textWant) => {
        ...
        const el = els.find((node) => {
            const t = String(node instanceof HTMLInputElement ? node.value : node.textContent || "")
                .replace(/\s+/g, " ")
                .trim()
                .toLowerCase();
        ...
        return true;
    }, candidate.text || "Pesquisar"), 3000, false);
    await sleepNode(150);
    // Re-lê rect após scroll.
    const fresh = await withNodeTimeout(findSearchButtonCandidatesFast(page), 3000, null);
 ...
 */
async function findSearchButtonCandidatesFast(page) {
    return page.evaluate(() => {
        const clean = (v) => String(v || "")
            .replace(/\s+/g, " ")
            .trim();
        ...
        const out = [];
        for (const raw of Array.from(document.querySelectorAll(selector))) {
            if (!(raw instanceof HTMLElement) || !isVisible(raw))
                continue;
            const text = raw instanceof HTMLInputElement ? clean(raw.value) : clean(raw.textContent).slice(0, 80);
            const aria = clean(raw.getAttribute("aria-label"));
            const title = clean(raw.getAttribute("title"));
            ...
            if (nt.includes("buscar"))
                score += 50;
            if (normalize(aria).includes("pesquisar") || normalize(aria).includes("buscar"))
                score += 60;
            if (raw.tagName === "BUTTON")
                score += 30;
            if (raw.getAttribute("type") === "submit")
                score += 25;
...
function probeFromCandidates(candidates, urlHint = "") {
 ...
 */
async function dispatchSearchWithAck(page, onProgress) {
    onProgress?.("SEARCH: descoberta rápida do CTA (fast)…");
    // NÃO usar probeSearchState completo no pré-clique (scan pesado → CDP trava >8s).
    const fast = await withNodeTimeout(findSearchButtonCandidatesFast(page), 5000, null);
    if (fast === null) {
        // CDP realmente não respondeu — mesma Page não serve; pede Chromium novo.
        throw new LeadsScrapeError("CDP_PROBE_TIMEOUT", "new-browser", "Probe rápido do CTA não respondeu em 5s (CDP/DOM travado) — reconectar Chromium.");
    }
    ...
    if (!before.searchButtonFound) {
        ...
        const full = await withNodeTimeout(findSearchButtonCandidates(page), 6000, null);
        if (full === null) {
            throw new LeadsScrapeError("CDP_PROBE_TIMEOUT", "new-browser", "Descoberta completa do CTA não respondeu em 6s — reconectar Chromium.");
        }
    ...
    if (!before.searchButtonFound || !before.searchButtonCandidate) {
        ...
        throw new LeadsScrapeError("SEARCH_BUTTON_NOT_FOUND", "same-page", `CTA de pesquisa não encontrado no DOM. ${formatProbeShort(before)}`);
    }
    const winner = before.searchButtonCandidate;
    onProgress?.(`SEARCH: snapshot — btn=true/${before.searchButtonCount} · #1 ${winner.tag} text="${winner.text.slice(0, 48)}" score=${winner.score}`);
    ...
    await withNodeTimeout(page.keyboard.press("Enter").then(() => undefined), 1500, undefined);
    ...
    const last = (await withNodeTimeout(probeSearchState(page), 3000, null)) || before;
    console.log(JSON.stringify({
...
async function readCnaeSelectedCount(page) {
    return withNodeTimeout(page.evaluate(() => {
        const root = document.querySelector('[role="dialog"], .modal, .o-modal') ||
...
async function selectAtividadePrincipalCnae(page, rawCode, onProgress, shouldAbort) {
    ...
    const evalSoft = async (fn, ms, fallback) => withNodeTimeout(fn(), ms, fallback);
    await dismissBlockingPortalOverlays(page).catch(() => undefined);
    await evalSoft(() => page.evaluate(() => {
    ...
    const opened = await evalSoft(() => page.evaluate(() => {
        const visible = (el) => {
            const h = el;
    ...
    if (!opened)
        throw new Error(`CNAE ${code}: CDP travou ao abrir modal.`);
    report("aguardando campo de busca…");
    let searchReady = false;
    for (let poll = 0; poll < 16; poll += 1) {
        if (aborted())
            throw new Error(`CNAE ${code}: abortado.`);
        searchReady = await evalSoft(() => page.evaluate(() => {
            const visible = (el) => {
                const h = el;
                const s = window.getComputedStyle(h);
    ...
    const typed = await evalSoft(() => page.evaluate((cnae) => {
        const visible = (el) => {
            const h = el;
            const s = window.getComputedStyle(h);
            return s.display !== "none" && s.visibility !== "hidden";
    ...
    for (let poll = 0; poll < 20; poll += 1) {
        if (aborted())
            throw new Error(`CNAE ${code}: abortado.`);
        marked = await evalSoft(() => page.evaluate((cnae) => {
...
async function setCheckboxByLabel(page, label, checked) {
    ...
    await withNodeTimeout(page.evaluate(({ needleIn, want }) => {
        ...
        return true;
    }, { needleIn: needle, want: checked }), 3000, false);
}
/**
...
async function setToggleByLabel(page, label, enabled) {
    ...
    await withNodeTimeout(page.evaluate(({ needle, want }) => {
        const labels = Array.from(document.querySelectorAll("label"));
        const lab = labels.find((el) => {
            const t = String(el.textContent || "")
                .replace(/\s+/g, " ")
...
async function enableTogglesFast(page, labels) {
    ...
    if (!needles.length)
        return;
    await withNodeTimeout(page.evaluate((needlesIn) => {
        const labels = Array.from(document.querySelectorAll("label"));
...
async function scrapeCasaDosDadosLeads(filters, onProgress, options) {
    ...
    const poolResumeFloor = Math.max(1, Math.round(Number(options?.resumeFloorPage || 1) || 1) || 1);
    /** Piso que sobe só com páginas realmente arquivadas nesta sessão (recover). */
    let liveResumeFloor = poolResumeFloor;
    let resumeFrom = resolvePortalResumePage(Math.max(1, Math.round(Number(options?.resumeFromPage || 1) || 1)), liveResumeFloor);
    let lastErr;
    let sessionStorageState = options?.storageState;
    if (!sessionStorageState) {
        sessionStorageState = (0, waba_leads_cnpj_browser_runtime_1.loadCasaDosDadosStorageState)();
    }
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
        try {
            ...
            if (attempt > 1) {
                ...
                onProgress?.(`COPY: recover tentativa ${attempt}/${maxAttempts} · retomando página ${resumeFrom} (piso ${liveResumeFloor})…`);
                ...
                onProgress?.(`COPY: retomando extração na página ${resumeFrom} (piso pool ${liveResumeFloor})…`);
            }
            return await scrapeCasaDosDadosLeadsOnce(filters, onProgress, {
                ...
                onPageCheckpoint: async (ckpt) => {
                    ...
                    if (resumeFrom > 1 && sequential > resumeFrom + 3) {
                        onProgress?.(`COPY: ignorando salto de checkpoint ${resumeFrom}→${sequential} (mantém ${resumeFrom})…`);
                        return;
                    }
            ...
            resumeFrom = resolvePortalResumePage(resumeFrom, liveResumeFloor);
            onProgress?.(`COPY: recover — falha de sessão; checkpoint página ${resumeFrom} (piso ${liveResumeFloor})…`);
...
async function scrapeCasaDosDadosLeadsOnce(filters, onProgress, options) {
    ...
    try {
        const runSession = async () => {
            ...
            if (wantFastResume) {
                ...
                if (lite && (lite.pagination || lite.cnpjNodes > 0)) {
                    ...
                    throw new LeadsScrapeError("RESUME_NO_RESULTS", "new-browser", `Retomada pág. ${resumeTarget} sem resultados na sessão — reabrir Chromium sem reaplicar CNAE/LOGIN.`);
                }
            }
            if (!usedFastResume) {
        ...
        if ((0, waba_leads_cnpj_browser_runtime_1.isDedicatedJobBrowser)(browser)) {
            await withNodeTimeout((0, waba_leads_cnpj_browser_runtime_1.releaseJobBrowser)(browser, "job-session-end").then(() => true), 8000, false);
        }
    }
}
