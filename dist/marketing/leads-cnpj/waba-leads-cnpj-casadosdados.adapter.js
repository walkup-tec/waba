...
async function selectAtividadePrincipalCnae(page, rawCode) {
    ...
    await page.evaluate(() => {
        ...
        const nodes = Array.from(document.querySelectorAll("label, span, p, strong, button, div, a"));
        const lab = nodes.find((el) => /Atividade\s+Principal\s*\(CNAE\)/i.test((el.textContent || "").replace(/\s+/g, " ")));
        if (lab && visible(lab))
            lab.click();
        const following = lab?.closest(".field, .control, .column, form, section")?.querySelector("input, button, .dropdown, .autocomplete, .taginput, .select");
        if (following && visible(following))
            following.click();
    });
    ...
    const typed = await page.evaluate((cnae) => {
        ...
        const score = (el) => {
            ...
            if (/atividade|cnae|c[oó]digo/.test(ph))
                s += 4;
            if (el.closest('[role="dialog"], .modal, .o-modal, .modal-card'))
                s += 6;
            if (/selecionados/i.test(document.body?.innerText || ""))
                s += 1;
            if (!visible(el))
                s -= 10;
            return s;
        ...
        return { ok: true, placeholder: String(target.placeholder || "") };
    }, code);
    if (!typed || !typed.ok) {
        const placeholders = await page.evaluate(() => Array.from(document.querySelectorAll("input[placeholder]"))
            ...
            .filter(Boolean));
        throw new Error(`CNAE: campo de busca ausente no portal. Código: ${code}. Placeholders: ${placeholders.length ? placeholders.join(" | ") : "(nenhum)"}.`);
    }
    await page.waitForTimeout(900);
    // 3) Marcar opção
    let marked = await page.evaluate((cnae) => {
        const boxes = Array.from(document.querySelectorAll('input[type="checkbox"]'));
        const box = boxes.find((b) => String(b.id || "").includes(cnae) ||
    ...
    if (!selectedState || selectedState < 1) {
        throw new Error(`CNAE ${code}: opção não marcada após digitar no modal (DOM).`);
    }
    // 4) Fechar
    await page.evaluate(() => {
...
async function scrapeCasaDosDadosLeads(filters, onProgress, options) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
        try {
            ...
            lastErr = error;
            const msg = error instanceof Error ? error.message : String(error);
            onProgress?.(`Abrindo Portal: interrupção na página ${resumeFrom} (${msg.slice(0, 140)}). Registros já arquivados serão mantidos.`);
            if (attempt >= maxAttempts)
                break;
            await new Promise((r) => setTimeout(r, Math.min(20000, 2000 * attempt)));
        }
    }
...
async function scrapeCasaDosDadosLeadsOnce(filters, onProgress, options) {
    ...
    try {
        ...
        await page
            .waitForFunction(() => /retornou\s+[\d.]+\s+empresas/i.test(document.body.innerText) ||
            /\d{2}\.\d{3}\.\d{3}\/\d{4}-\d{2}\s+-\s+/.test(document.body.innerText), { timeout: 45000 })
            .catch(() => null);
        ...
        const goToNextResultsPage = async (previousFirstCnpj, fromPage) => {
            ...
            for (const sel of nextCandidates) {
                ...
                const changed = await page
                    .waitForFunction(({ prev, expectedPage }) => {
                    const active = document.querySelector('nav[data-oruga="pagination"] button.pagination-link.is-current, nav[data-oruga="pagination"] button[aria-current="true"]');
                    const n = Number(String(active?.textContent || "").trim());
                    if (Number.isFinite(n) && n === expectedPage)
                        return true;
                ...
                }, { prev: previousFirstCnpj, expectedPage: targetPage }, { timeout: 25000 })
                    .then(() => true)
                    .catch(() => false);
                if (changed)
                    return true;
            ...
            const tryClickPage = async () => {
                ...
                for (const loc of candidates) {
                    try {
                        ...
                        await page.waitForTimeout(600);
                        const changed = await page
                            .waitForFunction(({ prevCnpj, expectedPage }) => {
                            const curBtn = document.querySelector('nav[data-oruga="pagination"] button.pagination-link[aria-current="page"], nav[data-oruga="pagination"] li[aria-current="page"] button, nav[data-oruga="pagination"] button.is-current');
                            const curText = String(curBtn?.textContent || "").trim();
                            const curNum = Number(curText.replace(/\D/g, ""));
                            if (Number.isFinite(curNum) && curNum === expectedPage)
                                return true;
                            const re = /^(\d{2}\.\d{3}\.\d{3}\/\d{4}-\d{2})\s+-\s+/m;
                            const m = String(document.body?.innerText || "").match(re);
                            if (!m || !prevCnpj)
                        ...
                        }, { prevCnpj: prev, expectedPage: target }, { timeout: 25000 })
                            .then(() => true)
                            .catch(() => false);
                        if (changed)
                            return true;
        ...
        for (let pageIndex = startPage; pageIndex <= pagesToFetch; pageIndex += 1) {
            ...
            if (added === 0 && pageIndex > 1) {
                onProgress?.(`Copiando: página ${pageIndex} sem CNPJs novos — encerrando paginação.`);
                break;
            }
