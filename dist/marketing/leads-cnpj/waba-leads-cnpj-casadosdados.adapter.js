...
async function dispatchSearchWithAck(page, onProgress) {
    const before = await withNodeTimeout(probeSearchState(page), 8000, null);
    if (!before) {
        throw new LeadsScrapeError("SEARCH_BUTTON_NOT_FOUND", "same-page", "Probe pré-clique não respondeu em 8s (CDP/DOM travado).");
    }
