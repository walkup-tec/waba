...
class WabaLeadsCnpjService {
    ...
    async runJob(listId) {
        ...
        try {
            ...
            const resumeEnrichment = list.status === "enriching" && Array.isArray(list.leads) && list.leads.length > 0;
            const resumeQueued = list.status === "queued" && Array.isArray(list.leads) && list.leads.length > 0;
            let dayLeads = [];
