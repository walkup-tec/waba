...
exports.resolveDailyEnrichLimit = resolveDailyEnrichLimit;
exports.normalizeAtividadePrincipalCnae = normalizeAtividadePrincipalCnae;
exports.parseLeadsCnpjFilters = parseLeadsCnpjFilters;
const waba_leads_cnpj_repository_1 = require("./waba-leads-cnpj.repository");
const waba_leads_cnpj_excel_service_1 = require("./waba-leads-cnpj-excel.service");
const waba_leads_cnpj_casadosdados_adapter_1 = require("./waba-leads-cnpj-casadosdados.adapter");
const waba_leads_cnpj_enrichment_adapter_1 = require("./waba-leads-cnpj-enrichment.adapter");
...
function isLeadsCnpjSearchRunning(status) {
...
function resolveScrapeHistoryMetrics(list, poolPending, usedCount) {
    ...
    const volumePages = cnpjCopied > 0 ? Math.min(pagesTotal, Math.max(0, Math.ceil(cnpjCopied / 20))) : 0;
    let ckptPages = 0;
    if (nextPage > 0) {
...
function campaignBaseName(name) {
    return String(name || "")
        .trim()
        .replace(/\s*·\s*\d{4}-\d{2}-\d{2}\s*$/, "")
        .trim();
}
function buildCampaignKey(baseName, source) {
 ...
 */
function resolveDailyEnrichLimit() {
...
function normalizeAtividadePrincipalCnae(value) {
...
function parseLeadsCnpjFilters(raw) {
    ...
    const situacaoCadastral = situacaoRaw
        .map((s) => String(s))
        .filter((s) => SITUACOES.has(s));
    const tipo = String(input.tipoPesquisa || "Exata").trim();
    const tipoPesquisa = (tipo === "Aproximada" ? "Aproximada" : "Exata");
...
function parseManualLeads(raw) {
    if (Array.isArray(raw)) {
        const leads = [];
        for (const item of raw) {
            if (typeof item === "string") {
                const cnpj = (0, waba_leads_cnpj_repository_1.normalizeCnpjDigits)(item);
                if (cnpj.length === 14)
                    leads.push((0, waba_leads_cnpj_repository_1.emptyLeadFromCnpj)(cnpj));
                continue;
            }
            if (!item || typeof item !== "object")
                continue;
            const row = item;
            const cnpj = (0, waba_leads_cnpj_repository_1.normalizeCnpjDigits)(row.cnpj || row.CNPJ);
            if (cnpj.length !== 14)
                continue;
            leads.push({
                cnpj,
    ...
    return dedupeLeads(matches.map((m) => (0, waba_leads_cnpj_repository_1.emptyLeadFromCnpj)(m)).filter((l) => l.cnpj.length === 14));
}
function dedupeLeads(leads) {
    const map = new Map();
    for (const lead of leads) {
        if (!map.has(lead.cnpj))
            map.set(lead.cnpj, lead);
    }
    return [...map.values()];
}
class WabaLeadsCnpjService {
    constructor() {
        this.repository = new waba_leads_cnpj_repository_1.WabaLeadsCnpjRepository();
    }
    listSummaries() {
    ...
    listHistory() {
        ...
        const downloadsByCampaign = new Map();
        for (const list of lists) {
            if (list.status !== "ready" || !list.exportFileName)
                continue;
            const key = String(list.campaignKey || "").trim();
            if (!key)
                continue;
            const idx = list.listaIndex != null && Number(list.listaIndex) > 0
                ? Math.round(Number(list.listaIndex))
                : 1;
            const row = {
                id: list.id,
    ...
    getEnrichQueueSummary() {
        ...
        for (const list of this.repository.list()) {
            ...
            if (key && !nameByKey.has(key))
                nameByKey.set(key, campaignBaseName(list.name));
        }
        const entries = order.map((key, i) => {
            let state = "idle";
            if (q.activeCampaignKey && key === q.activeCampaignKey)
                state = "active";
            else if (q.lastCompletedCampaignKey && key === q.lastCompletedCampaignKey)
                state = "done";
            else if (work.has(key))
                state = "waiting";
            return {
                key,
    ...
    getDownload(id) {
        ...
        if (!list || list.status !== "ready" || !list.exportFileName)
            return null;
        ...
        const idx = list.listaIndex != null && Number(list.listaIndex) > 0
            ? Math.round(Number(list.listaIndex))
            : 1;
        const base = (0, waba_leads_cnpj_excel_service_1.sanitizeExportBaseName)(campaignBaseName(list.name));
        return {
            filePath,
    ...
    isCampaignPortalCopyComplete(campaignKey) {
        ...
        return this.repository.list().some((l) => {
            if (String(l.campaignKey || "").trim() !== key)
                return false;
            if (l.scrapeCompleted !== true)
                return false;
     ...
     */
    isPortalScrapeReallyComplete(list, poolPending, usedCount, portalUiMaxPage = (0, waba_leads_cnpj_casadosdados_adapter_1.resolvePortalUiMaxPage)()) {
        if (list.scrapeCompleted !== true)
            return false;
        const reason = String(list.scrapeDoneReason || "").trim().toUpperCase();
        if (reason === "THREE_EMPTY_PAGES" ||
            reason === "EMPTY_AT_END" ||
            reason === "SEARCH_EMPTY" ||
            reason === "UI_MAX_PAGE") {
            return true;
        }
        const maxWanted = Math.max(1, Math.round(Number(list.filters?.maxPages || 0) || 0) || portalUiMaxPage);
    ...
    resumeIncompletePortalScrape(id) {
        ...
        const updated = this.repository.update({
            ...
            scrapeCheckpoint: {
                ...
                pagesToFetch: Math.max(1, Math.round(Number(list.filters?.maxPages || 0) || 0) || portalUiMaxPage),
     ...
     */
    finalizeEnrichDayNow(id, options) {
        const listId = String(id || "").trim();
        const list = this.repository.getById(listId);
        if (!list)
            throw new Error("Lista não encontrada.");
        ...
        cancelledJobs.add(listId);
        const leads = Array.isArray(list.leads) ? list.leads : [];
        const enrichedLeads = leads.filter((l) => (0, waba_leads_cnpj_enrichment_adapter_1.leadLooksEnriched)(l));
        if (!enrichedLeads.length) {
            cancelledJobs.delete(listId);
            throw new Error("Nenhum CNPJ enriquecido ainda para gerar a lista do dia.");
        }
        const enrichedSet = new Set(enrichedLeads.map((l) => (0, waba_leads_cnpj_repository_1.normalizeCnpjDigits)(l.cnpj)));
        const leftover = leads.filter((l) => !enrichedSet.has((0, waba_leads_cnpj_repository_1.normalizeCnpjDigits)(l.cnpj)));
        if (leftover.length) {
            const used = this.repository.collectUsedCnpjs(campaignKey);
            for (const lead of enrichedLeads)
                used.add((0, waba_leads_cnpj_repository_1.normalizeCnpjDigits)(lead.cnpj));
            this.repository.mergePool({
                key: campaignKey,
                name: baseName,
                source: list.source,
        ...
        const forExport = (0, waba_leads_cnpj_excel_service_1.expandLeadsByMobileForEvo)(enrichedLeads);
    ...
    removeCampaignFromEnrichOrder(campaignKey) {
        ...
        this.repository.saveEnrichQueue({
            ...
            lastCompletedCampaignKey: q.lastCompletedCampaignKey === key ? null : q.lastCompletedCampaignKey,
        });
    }
    /** Campanhas com trabalho de enriquecimento pendente (lote ou pool). */
    campaignsWithEnrichWork() {
        const keys = new Set();
        for (const list of this.repository.list()) {
            const key = String(list.campaignKey || "").trim();
            if (!key)
    ...
    nextCampaignAfter(order, lastKey) {
        if (!order.length)
            return null;
        if (!lastKey)
            return order[0];
        ...
        for (let i = 1; i <= order.length; i += 1) {
            ...
            if (this.campaignsWithEnrichWork().includes(candidate))
                return candidate;
        }
        return null;
    }
    ...
    queuePositionLabel(campaignKey) {
        const q = this.repository.getEnrichQueue();
        const order = q.order.length ? q.order : this.rebuildEnrichOrder();
        ...
        if (campaignKey === active)
            return "Enriquecimento do dia (fila ativa)…";
        const activeIdx = active ? order.indexOf(active) : -1;
        let pos = 0;
        for (let i = 1; i <= order.length; i += 1) {
            const k = order[(Math.max(0, activeIdx) + i) % order.length];
            if (!this.campaignsWithEnrichWork().includes(k))
                continue;
            pos += 1;
            if (k === campaignKey) {
    ...
    pauseNonActiveEnrichLists(activeCampaignKey) {
        for (const list of this.repository.list()) {
            if (list.status !== "enriching" && list.status !== "queued")
                continue;
            const key = String(list.campaignKey || "").trim();
            ...
            if (activeCampaignKey && key === activeCampaignKey) {
                if (list.status === "queued" && Array.isArray(list.leads) && list.leads.length) {
                    this.repository.update({
                        ...list,
                        status: "enriching",
                        progressMessage: (0, waba_leads_cnpj_enrichment_adapter_1.formatReceitaWsLegend)(list.leads.filter((l) => l.enriched).length, list.leads.length, "retomando fila…"),
                        updatedAt: new Date().toISOString(),
                    });
                }
                continue;
    ...
    armGlobalEnrichQueue() {
        if (globalEnrichTimer)
            return;
        const tick = () => {
    ...
    syncEnrichQueueDay() {
        ...
        if (q.dayKey !== today) {
            ...
            if (prevActive) {
                const prevList = this.repository.list().find((l) => String(l.campaignKey || "") === prevActive &&
                    (l.status === "enriching" || l.status === "queued") &&
                    Array.isArray(l.leads) &&
                    l.leads.length > 0);
                if (prevList && prevList.leads.some((l) => (0, waba_leads_cnpj_enrichment_adapter_1.leadLooksEnriched)(l))) {
                    try {
    ...
    startActiveCampaignEnrichDay(activeCampaignKey) {
        ...
        if (existingToday) {
            if ((existingToday.status === "enriching" || existingToday.status === "queued") &&
        ...
        const staleOpen = this.repository
            .list()
            .filter((l) => String(l.campaignKey || "") === active &&
            (l.status === "enriching" || l.status === "queued") &&
            String(l.dayKey || "") !== today);
        for (const stale of staleOpen) {
            if ((stale.leads || []).some((row) => (0, waba_leads_cnpj_enrichment_adapter_1.leadLooksEnriched)(row))) {
                try {
    ...
    returnListLeadsToPoolAndClose(list, progressMessage) {
    ...
    closeOverdueEnrichDays() {
        const today = saoPauloDayKey();
        const overdue = this.repository
            .list()
            .filter((l) => (l.status === "enriching" || l.status === "queued") &&
            String(l.dayKey || "") &&
            String(l.dayKey) < today &&
            Array.isArray(l.leads) &&
            l.leads.some((row) => (0, waba_leads_cnpj_enrichment_adapter_1.leadLooksEnriched)(row)))
    ...
    hasActiveEnrichWork() {
        const q = this.repository.getEnrichQueue();
        if (q.activeCampaignKey) {
            return this.repository.list().some((l) => String(l.campaignKey || "") === q.activeCampaignKey &&
                (l.status === "enriching" || l.status === "queued"));
        }
        return this.repository.list().some((l) => l.status === "enriching");
    }
    canEnrichCampaignToday(campaignKey) {
        const key = String(campaignKey || "").trim();
        if (!key)
            return false;
        const active = this.ensureEnrichSlotForToday();
    ...
    rebuildExportWithEvoPhones(id) {
        ...
        const dayKey = list.dayKey || saoPauloDayKey();
        const campaignKey = String(list.campaignKey || "").trim();
        const forExport = (0, waba_leads_cnpj_excel_service_1.expandLeadsByMobileForEvo)(list.leads);
        const withEvo = forExport.filter((l) => (0, waba_leads_cnpj_excel_service_1.isEvoBrazilMobileDigits)(l.telefone)).length;
        const listaIndex = list.listaIndex != null && Number(list.listaIndex) > 0
            ? Math.round(Number(list.listaIndex))
            : this.repository.nextListaIndex(campaignKey);
        const exportFileName = this.repository.saveExportFile(listId, (0, waba_leads_cnpj_excel_service_1.buildLeadsCnpjExcelBuffer)(forExport), (0, waba_leads_cnpj_excel_service_1.sanitizeExportBaseName)(`${baseName}-${formatListaLabel(listaIndex)}`), dayKey);
        const updated = this.repository.update({
            ...list,
            leads: forExport,
            leadCount: forExport.length,
     ...
     */
    async refreshPhonesAndRebuildExcel(id) {
        const listId = String(id || "").trim();
        if (!listId || phoneRefreshJobs.has(listId))
            return null;
        const list = this.repository.getById(listId);
        if (!list || list.status !== "ready" || !Array.isArray(list.leads) || !list.leads.length) {
            return null;
        ...
        try {
            ...
            for (let i = 0; i < out.length; i += 1) {
                ...
                try {
                    const next = await (0, waba_leads_cnpj_enrichment_adapter_1.enrichViaReceitaWs)(current.cnpj, apiKey);
                ...
                if (fetched > 0 && fetched % CHECKPOINT_EVERY === 0) {
                    const partialExport = (0, waba_leads_cnpj_excel_service_1.expandLeadsByMobileForEvo)(out);
                    const withEvoPartial = partialExport.filter((l) => (0, waba_leads_cnpj_excel_service_1.isEvoBrazilMobileDigits)(l.telefone)).length;
                    const listaIndexPartial = list.listaIndex != null && Number(list.listaIndex) > 0
                        ? Math.round(Number(list.listaIndex))
                        : this.repository.nextListaIndex(campaignKey);
                    const exportPartial = this.repository.saveExportFile(listId, (0, waba_leads_cnpj_excel_service_1.buildLeadsCnpjExcelBuffer)(partialExport), (0, waba_leads_cnpj_excel_service_1.sanitizeExportBaseName)(`${baseName}-${formatListaLabel(listaIndexPartial)}`), dayKey);
                    this.repository.update({
                        ...this.repository.getById(listId),
                        leads: out.map((l) => ({ ...l })),
            ...
            const forExport = (0, waba_leads_cnpj_excel_service_1.expandLeadsByMobileForEvo)(out);
            const withEvo = forExport.filter((l) => (0, waba_leads_cnpj_excel_service_1.isEvoBrazilMobileDigits)(l.telefone)).length;
            const excel = (0, waba_leads_cnpj_excel_service_1.buildLeadsCnpjExcelBuffer)(forExport);
            const listaIndex = list.listaIndex != null && Number(list.listaIndex) > 0
                ? Math.round(Number(list.listaIndex))
    ...
    createAndStart(input) {
        ...
        if (baseName.length > 100)
            throw new Error("Nome da lista deve ter no máximo 100 caracteres.");
        const source = input.source === "manual" ? "manual" : "portal";
        ...
        const reuseToday = existingToday?.status === "failed" || existingToday?.status === "stopped";
        ...
        const list = {
            ...
            scrapeCheckpoint: reuseToday && existingToday ? existingToday.scrapeCheckpoint ?? null : null,
            scrapeReconnectAttempts: 0,
    ...
    clearContinueTimer(campaignKey) {
        const key = String(campaignKey || "").trim();
        if (!key)
            return;
    ...
    armContinueFromPool(campaignKey) {
        const key = String(campaignKey || "").trim();
        if (!key)
            return;
        if (this.repository.getPool(key)?.autoContinuePaused)
            return;
        if (continueTimers.has(key))
            return;
        const tick = () => {
            continueTimers.delete(key);
            try {
     ...
     */
    tryStartNextDayFromPool(campaignKey) {
        const pool = this.repository.getPool(campaignKey);
        if (!pool || !(pool.pending || []).length)
            return false;
        if (pool.autoContinuePaused)
            return false;
        const busy = this.repository
            .list()
            .some((l) => l.campaignKey === campaignKey &&
    ...
    async runJob(listId) {
        ...
        const patch = (partial, options) => {
            ...
            if (partial.leads)
                next.leadCount = partial.leads.length;
            const onlyProgress = partial.progressMessage !== undefined &&
        ...
        try {
            ...
            if (prematurePortalCopy && Array.isArray(list.leads) && list.leads.length > 0) {
                ...
                patch({
                    ...
                    scrapeCheckpoint: {
                        nextPage: resumePage,
                        portalTotal: list.scrapeCheckpoint?.portalTotal ?? null,
                        pagesToFetch: Math.max(1, Math.round(Number(list.filters?.maxPages || 0) || 0) || (0, waba_leads_cnpj_casadosdados_adapter_1.resolvePortalUiMaxPage)()),
            ...
            if (resumeEnrichment || resumeQueued) {
                ...
                patch({
                    status: "enriching",
                    progressMessage: (0, waba_leads_cnpj_enrichment_adapter_1.formatReceitaWsLegend)(already, dayLeads.length, already > 0 ? "retomando…" : "iniciando…"),
                });
            }
            else {
                if (list.source === "portal" && !list.skipPortalScrape) {
                    ...
                    const mustResumeScrape = Boolean(freshList.scrapeCheckpoint &&
                    ...
                    if (needPortalCopy || mustResumeScrape) {
                        ...
                        const pagesToFetchTarget = Number(list.filters?.maxPages) > 0
                                ...
                                onPageCheckpoint: async (c) => {
                                    ...
                                    patch({
                                        ...
                                        progressMessage: beyondUi
                                            ...
                                            : `COPY: página ${c.completedPage}/${c.pagesToFetch}${c.portalTotal != null
                        ...
                        if (scrapeSessionCompleted) {
                            ...
                            if (!this.isPortalScrapeReallyComplete(provisional, afterMerge, used.size, portalUiMaxPage)) {
                ...
                if (list.source === "portal" && !list.skipPortalScrape) {
                    ...
                    if (!campaignCopyDone) {
                        patch({
                            status: "scraping",
                            scrapeCompleted: false,
                            progressMessage: "Cópia do portal incompleta — enriquecimento ReceitaWS só após todas as páginas (até maxPages/teto UI).",
                            error: null,
                        });
                        setTimeout(() => {
                            const again = this.repository.getById(listId);
                ...
                if (!taken.length) {
                    throw new Error("Nenhum CNPJ novo para o lote de hoje (pool vazio após raspagem/paginação, ou todos já usados em listas anteriores).");
                }
                remaining = this.repository.getPool(campaignKey)?.pending.length || 0;
                dayLeads = taken.map((item) => {
                    const lead = (0, waba_leads_cnpj_repository_1.emptyLeadFromCnpj)(item.cnpj);
                    lead.nome = item.nome;
                    return lead;
                });
                list = patch({
                    ...
                    scrapeCheckpoint: null,
                    scrapeReconnectAttempts: 0,
                    progressMessage: (0, waba_leads_cnpj_enrichment_adapter_1.formatReceitaWsLegend)(0, dayLeads.length, "iniciando…"),
                });
                if (!list)
                    return;
            }
            const enriched = await (0, waba_leads_cnpj_enrichment_adapter_1.enrichLeadsCnpjList)(listId, dayLeads, (message) => {
                assertAlive();
            ...
            const forExport = (0, waba_leads_cnpj_excel_service_1.expandLeadsByMobileForEvo)(enriched);
            const excel = (0, waba_leads_cnpj_excel_service_1.buildLeadsCnpjExcelBuffer)(forExport);
            const listaIndex = this.repository.nextListaIndex(campaignKey);
            const exportFileName = this.repository.saveExportFile(listId, excel, (0, waba_leads_cnpj_excel_service_1.sanitizeExportBaseName)(`${baseName}-${formatListaLabel(listaIndex)}`), dayKey);
            ...
            const msg = error instanceof Error ? error.message : "Falha ao processar a lista de leads CNPJ.";
            ...
            if ((0, waba_leads_cnpj_casadosdados_adapter_1.isSoftScrapeError)(error) || ((0, waba_leads_cnpj_casadosdados_adapter_1.isLeadsScrapeError)(error) && error.recovery !== "new-browser")) {
                const code = (0, waba_leads_cnpj_casadosdados_adapter_1.isLeadsScrapeError)(error) ? error.code : "SOFT_SCRAPE";
                patch({
                    status: "failed",
                    error: msg,
                    scrapeCompleted: false,
            ...
            const maxReconnect = Math.max(1, Math.round(Number(process.env.CASADOSDADOS_JOB_RECONNECTS || 20) || 20));
            ...
            const hardRecovery = ((0, waba_leads_cnpj_casadosdados_adapter_1.isLeadsScrapeError)(error) && error.recovery === "new-browser") ||
                (!(error instanceof waba_leads_cnpj_casadosdados_adapter_1.LeadsScrapeError) &&
                    /Target crashed|Page crashed|net::ERR_ABORTED|frame was detached|browser has been closed|has been closed|RENDERER_UNRESPONSIVE|BROWSER_DISCONNECTED|CDP_PROBE_TIMEOUT|LOGIN_TIMEOUT|ANTI_BOT|anti-bot|cloudflare|turnstile|just a moment|um momento|locator\.waitFor|input\[name=.email/i.test(msg));
...
exports.WabaLeadsCnpjService = WabaLeadsCnpjService;
