...
let portalScrapeActiveListId = null;
...
function portalScrapeQueuePosition(listId) {
    ...
    if (waitingIdx >= 0)
        return waitingIdx + 1;
    if (portalScrapeActiveListId === listId)
        return 0;
...
function acquirePortalScrapeSlot(listId, onWaiting) {
    return new Promise((resolve, reject) => {
        ...
        waiter.pulse = setInterval(() => {
            ...
            if (pos > 0)
                onWaiting?.(pos, portalScrapeActiveListId);
        }, 4000);
    });
}
            ...
            if (resumeEnrichment || resumeQueued) {
                ...
                if (list.source === "portal" && !list.skipPortalScrape) {
                    ...
                    if (mustResumeScrape || (!freshList.scrapeCheckpoint && pendingCount < dailyLimit && !(ckpt && resumeFromPage > portalUiMaxPage))) {
                        const scrapeResumeFrom = Math.max(1, Math.round(Number(freshList.scrapeCheckpoint?.nextPage || 1) || 1));
                        const scrapeFilters = {
                            ...list.filters,
                        ...
                        try {
                            releaseScrapeSlot = await acquirePortalScrapeSlot(listId, (position, activeId) => {
                                patch({
                                    status: "scraping",
                                    progressMessage: `Fila de raspagem (1 Chromium por vez): posição ${position}${activeId ? ` — aguardando outro job` : ""}…`,
                                    error: null,
                                });
                            });
                            assertAlive();
