...
exports.staffEmailHasDisparoCloudAccess = staffEmailHasDisparoCloudAccess;
 ...
 */
function staffEmailHasDisparoCloudAccess(email, lookup = defaultLookup(), env = process.env) {
    const normalized = normalizeEmail(email);
    if (!normalized)
        return false;
    const user = lookup.getByEmail(normalized);
    ...
    if (user.role !== "operacional" && user.role !== "suporte")
        return false;
    return (0, waba_menu_permissions_service_1.isMenuAllowedForUser)(user, DISPARO_CLOUD_MENU_ID);
}
/**
...
function campaignAttendedByLaboratorioStaff(intake, lookup = defaultLookup(), env = process.env) {
    ...
    if (assigned)
        return staffEmailHasDisparoCloudAccess(assigned, lookup, env);
    const started = normalizeEmail(String(intake.startedByEmail || ""));
    if (started)
        return staffEmailHasDisparoCloudAccess(started, lookup, env);
    return false;
}
