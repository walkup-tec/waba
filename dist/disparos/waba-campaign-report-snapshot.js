...
function buildCampaignPieGradient(segments, totalLeads) {
    ...
    if (!stops.length)
        return "conic-gradient(#64748b 0deg 360deg)";
...
function formatTimelineWhen(item) {
    ...
    if (idx === -1)
        return { iso: "", full, date: full, time: "" };
    return { iso: "", full, date: full.slice(0, idx).trim(), time: full.slice(idx + 3).trim() };
}
function buildSubscriberCampaignTimelineHtml(timeline) {
    ...
    const note = escapeHtml(String(timeline?.metaCollectionNote || "").trim() || waba_campaign_report_timeline_1.META_REPORT_COLLECTION_NOTE);
    const lastIndex = items.length - 1;
    const steps = items
        .map((item, index) => {
        const label = escapeHtml(String(item.label || ""));
        const when = formatTimelineWhen({
            display: item.display || (0, waba_campaign_report_timeline_1.formatCampaignReportDateTime)(item.at),
            at: item.at,
        });
...
function buildCampaignPerformanceDashboardHtml(input) {
    ...
    const pieLegendHtml = pieLegend
        .map((item) => `
            ...
            `)
        .join("");
    const finalizedLegend = reportSource === "meta_lab"
...
function buildCampaignReportSnapshotHtml(input) {
    ...
    const body = buildSubscriberCampaignTimelineHtml(input.timeline) + buildCampaignPerformanceDashboardHtml(input);
    return `<!doctype html>
<html lang="pt-BR">
<head>
  <meta charset="utf-8" />
