...
const waba_adspower_routes_1 = require("./integrations/adspower/waba-adspower.routes");
...
const waba_leads_cnpj_routes_1 = require("./marketing/leads-cnpj/waba-leads-cnpj.routes");
const waba_operacional_campanhas_routes_1 = require("./admin/waba-operacional-campanhas.routes");
const waba_device_cloud_routes_1 = require("./device-cloud/waba-device-cloud.routes");
...
app.get("/service/evo-qr-create-smoke", async (_req, res) => {
    ...
    try {
        ...
        const createResult = await callEvoAction(`${EVO_API_BASE}/instance/create`, "POST", createPayload, { timeoutMs: Math.min((0, evo_http_client_1.defaultEvoHttpTimeoutMs)(), 30000), retries: 1 });
        const qrCode = tryExtractQrCode(createResult.json) || tryExtractQrCode(createResult.body);
        const deleteResult = await callEvoAction(`${EVO_API_BASE}/instance/delete/${encodeURIComponent(name)}`, "DELETE", undefined, { timeoutMs: 15000, retries: 1 });
...
(0, waba_adspower_routes_1.registerAdsPowerIngestRoute)(app);
...
(0, waba_adspower_routes_1.registerAdsPowerLabRoutes)(app);
...
(0, instance_integration_probe_1.setIntegrationProbeFinishedHandler)((status) => {
    ...
    void (async () => {
        if (instanceNameHeldByUnfinishedCampaign(status.sourceInstance)) {
            markCampaignChipUnsendable(status.sourceInstance, "restriction-probe-held");
            console.info(`[Campanha] ${status.sourceInstance} bloqueado para disparo (restrição com chip em campanha) — troca automática.`);
            return;
        }
...
(0, instance_inbound_validation_service_1.setInboundValidationFinishedHandler)((status) => {
    ...
    void (async () => {
        if (instanceNameHeldByUnfinishedCampaign(status.instanceName)) {
            ...
            console.info(`[Campanha] ${status.instanceName} bloqueado para disparo (restrição inbound com chip em campanha) — troca automática.`);
            return;
        }
...
function parseDisparosConfig(input) {
    ...
    return {
        ...
        instanceSlotCount: Math.floor(Number(input?.instanceSlotCount) || 0) >= 1
            ? Math.floor(Number(input.instanceSlotCount))
            : selectedDisparadorInstances.length,
...
async function resolveAquecedorDashboardScope(ownerEmail) {
    ...
    for (const name of names) {
        ...
        if (alias && alias !== normalized)
            filterValues.push(alias);
    }
    return { globalScope: false, instanceNames: names, filterValues };
}
function buildDadosScopeFingerprint(scope) {
    if (scope.globalScope)
        return "global";
    if (!scope.instanceNames.length)
        return "empty";
...
async function loadAquecedorTurnManager(supabase, connected) {
    ...
    for (const ev of events) {
        ...
        if (Number.isFinite(evAtMs))
            pairLastEventAtMs.set(pairKey, evAtMs);
    ...
    for (const [pairKey, lastAtMs] of pairLastEventAtMs) {
        if (lastAtMs >= pairTurnStaleBeforeMs)
            continue;
...
async function pickAquecedorCombinationAsync(supabase, connected, combinations, startIndex, ownerEmail, extraBlockedDirectedKeys) {
    ...
    if (extraBlockedDirectedKeys?.size) {
        for (const key of extraBlockedDirectedKeys)
            blocked.add(key);
    }
...
async function releaseStuckAquecedorQueueRows(supabase, _scopedInstanceNames) {
...
async function fetchProcessableAquecedorPending(supabase, scopedInstanceNames, preferredOrigem) {
...
async function ensureAquecedorPendingMessageOnce(pair) {
    ...
    const { data: orphan } = (await (supabase
        ...
        .maybeSingle()));
    if (orphan?.id) {
        const { error: claimError } = await supabase.from("aquecedor")
            .update({
            mensagem,
            scheduled_at: now,
            instancia: origem,
            ...
            .eq("status", "PENDENTE");
        if (!claimError)
            return { ok: true, pendingId: orphan.id };
    }
...
const campaignInstanceNextSendAt = new Map();
/** Envios confirmados nesta campanha, por instância (balanço). */
const campaignInstanceSendCounts = new Map();
...
const campaignBlockedInstanceUntilMs = new Map();
function campaignBlockedInstanceKey(instanceName) {
    return String(instanceName || "").trim().toLowerCase();
}
function markCampaignInstanceBlocked(instanceName, reason) {
    ...
    console.warn(`[Campanha] chip ${instanceName} tratado como bloqueado (${reason}) — troca automática.`);
}
/** Inapto a enviar: vermelho na campanha, tag Restrição persistida, entra na troca 1:1. */
function markCampaignChipUnsendable(instanceName, reason) {
    markCampaignInstanceBlocked(instanceName, reason);
    void (0, whatsapp_connecting_restriction_service_1.markWhatsappRestrictionExplicit)(instanceName, reason);
}
function isCampaignInstanceBlocked(instanceName) {
    const key = campaignBlockedInstanceKey(instanceName);
    if (!key)
        return false;
...
function isEvoSenderBanHttp(status, body) {
    if (Number(status) === 403)
        return true;
    const b = String(body || "").toLowerCase();
    return ((b.includes("statusreason") && b.includes("403")) ||
...
function getAquecedorWorkerId() {
    return aquecedor_owner_runtime_registry_1.AQUECEDOR_OWNER_WORKER_ID;
}
function computeAquecedorNextCycleDelayMs(runtime) {
...
function scheduleAquecedorCycleTick(ownerEmail) {
    const motor = (0, aquecedor_owner_runtime_registry_1.getAquecedorOwnerMotor)(ownerEmail);
    if (!motor.runtime.running || !ENABLE_AQUECEDOR_PROCESSING)
        return;
...
async function syncAquecedorWorkerLeadership() {
    if (!ENABLE_AQUECEDOR_PROCESSING || MAINTENANCE_MODE)
        return;
    await (0, aquecedor_owner_runtime_registry_1.reloadAquecedorOwnerMotorsFromDisk)(true);
    for (const ownerEmail of (0, aquecedor_owner_runtime_registry_1.listAquecedorOwnerEmails)()) {
        const motor = (0, aquecedor_owner_runtime_registry_1.getAquecedorOwnerMotor)(ownerEmail);
...
function startAquecedorRuntimeLocal(ownerEmail) {
    const motor = (0, aquecedor_owner_runtime_registry_1.getAquecedorOwnerMotor)(ownerEmail);
    ...
    if (motor.runtime.running && motor.scheduleTimer)
        return;
    motor.runtime.running = true;
    void runAquecedorCycle(ownerEmail).finally(() => scheduleAquecedorCycleTick(ownerEmail));
}
async function stopAquecedorRuntimeForOwner(ownerEmail) {
    const motor = (0, aquecedor_owner_runtime_registry_1.getAquecedorOwnerMotor)(ownerEmail);
    (0, aquecedor_owner_runtime_registry_1.stopAquecedorOwnerMotorLocal)(ownerEmail);
    motor.runtime.lastResult = "Aquecedor parado.";
    await (0, aquecedor_owner_runtime_registry_1.persistAquecedorOwnerIntent)(ownerEmail, false);
}
...
async function ensureAquecedorInstanceRegistered(instanceName, options) {
    try {
...
async function syncAquecedorConnectedInstances(supabase, connected) {
    ...
    if (toRegister.length) {
        ...
        for (const row of toRegister) {
            if (row.useAquecedor) {
                await (0, aquecedor_instance_lifecycle_service_1.registerAquecedorInstancePreparing)(row.instanceName, new Date().toISOString(), {
                    forceNewIntegration: true,
                });
            }
        }
...
function campaignInstanceGateKey(campaignId, instanceName) {
    return `${String(campaignId || "").trim()}::${String(instanceName || "").trim().toLowerCase()}`;
}
function getCampaignInstanceSendCount(campaignId, instanceName) {
...
function recordCampaignInstanceSend(campaignId, instanceName) {
    const camp = String(campaignId || "").trim();
    const key = String(instanceName || "").trim().toLowerCase();
    if (!camp || !key)
        return;
...
function isCampaignInstanceInCooldown(campaignId, instanceName) {
    const until = campaignInstanceNextSendAt.get(campaignInstanceGateKey(campaignId, instanceName)) || 0;
    return Date.now() < until;
}
function resolveSelectedDisparadorToEvoName(name, connected) {
    ...
    if (exact)
        return String(exact.instancia || "").trim() || raw;
    ...
    if (hits.length === 1)
        return String(hits[0].instancia || "").trim() || raw;
    return raw;
}
...
async function runAquecedorCycleTestBatch(connected, cicloGlobal, supabase, _config, ownerEmail) {
    ...
    if (aquecedorCycleMotor) {
        (0, aquecedor_owner_runtime_registry_1.setAquecedorOwnerCicloGlobal)(aquecedorCycleMotor, proximo);
    }
}
...
async function runAquecedorCycle(ownerEmail, forceTest = false) {
    ...
    try {
        ...
        const scopedKeys = new Set((await listAquecedorScopedInstanceNames(ownerEmail)).map((name) => name.toLowerCase()));
        const softSkipDirected = new Set();
        const maxPickAttempts = Math.min(AQUECEDOR_PICK_ATTEMPTS_MAX, Math.max(1, combinations.length));
        let picked = null;
        let chosen = null;
        ...
        for (let attempt = 0; attempt < maxPickAttempts; attempt += 1) {
            const candidate = await pickAquecedorCombinationAsync(supabase, connected, combinations, cicloGlobal + attempt, ownerEmail, softSkipDirected);
            if (!candidate)
                break;
            const dirKey = (0, delivery_cooldown_service_1.buildDirectedCooldownKey)(candidate.chosen.instancia_origem, candidate.chosen.instancia_destino);
            if (softSkipDirected.has(dirKey))
                continue;
            if (!scopedKeys.has(candidate.chosen.instancia_origem.toLowerCase()) ||
            ...
            const dailyQuota = await (0, aquecedor_instance_lifecycle_service_1.canAquecedorInstanceSendToday)(candidate.chosen.instancia_origem);
            if (!dailyQuota.ok) {
                softSkipDirected.add(dirKey);
                lastSoftReason = `${candidate.chosen.instancia_origem}: ${dailyQuota.reason}`;
                continue;
            }
            const turnCheck = await verifyAquecedorConversationTurn(supabase, connected, candidate.chosen.instancia_origem, candidate.chosen.instancia_destino);
            if (!turnCheck.ok) {
                softSkipDirected.add(dirKey);
                lastSoftReason = turnCheck.reason;
                continue;
            }
            if (await hasRecentAquecedorSendBetween(supabase, connected, candidate.chosen.instancia_origem, candidate.chosen.instancia_destino, 90)) {
                softSkipDirected.add(dirKey);
                lastSoftReason = `Envio ${candidate.chosen.instancia_origem} → ${candidate.chosen.instancia_destino} ignorado: duplicata recente.`;
                continue;
            }
            const openCheck = await assertAquecedorInstancesOpenForSend(candidate.chosen.instancia_origem, candidate.chosen.instancia_destino);
            if (!openCheck.ok) {
                softSkipDirected.add(dirKey);
                lastSoftReason = openCheck.reason;
                await (0, delivery_cooldown_service_1.recordDirectedDeliveryFailure)({
            ...
            const pending = await fetchProcessableAquecedorPending(supabase, scopedInstanceNames, candidate.chosen.instancia_origem);
            if (!pending?.id) {
                softSkipDirected.add(dirKey);
                lastSoftReason =
                ...
                if (!ensured.ok && isSupabaseTransientError({ message: lastSoftReason })) {
                    aquecedorCycleRuntime().nextAllowedAt = new Date(Date.now() + 60000).toISOString();
            ...
            pendingData = pending;
            texto = await resolveAquecedorMessageForSend(supabase, pending.id, String(pending.mensagem || ""), candidateContext);
            break;
        }
        if (!picked || !chosen || !pairContext || !pendingData?.id) {
            ...
            const skipHint = softSkipDirected.size
                ...
                : "";
            deferAquecedorRetryOrWindow(config, nowSp, blocked.size || softSkipDirected.size ? 45 : 30, lastSoftReason
        ...
        if (!numberCandidates.length) {
            ...
            deferAquecedorRetryOrWindow(config, nowSp, 120, `Destino ${chosen.instancia_destino} sem número WhatsApp válido para envio.`);
            return;
        }
        const origemConnected = connected.find((item) => item.instancia.toLowerCase() === chosen.instancia_origem.toLowerCase());
        ...
        for (let ni = 0; ni < numberCandidates.length; ni += 1) {
            if (sendAccepted)
                break;
            if (failedNumberTries >= MAX_FAILED_NUMBER_TRIES)
                break;
            ...
            if (!sendResult.ok) {
                failedNumberTries += 1;
                const detailStr = String(sendResult.json?.message ||
        ...
        if (!sendAccepted) {
            ...
            deferAquecedorRetryOrWindow(config, nowSp, lastSendStatus === 0 ? 180 : 120, `Falha no envio via EVO (HTTP ${lastSendStatus}). Mensagem voltou para pendente.`);
            aquecedorCycleRuntime().lastEvoError = {
...
function sendIndexHtml(res) {
    ...
    const html = (0, waba_sale_pricing_catalog_1.injectPublicPricingBootstrap)((0, base_path_1.injectRuntimeIntoIndexHtml)(loadIndexHtmlTemplate(), {
        basePath: base_path_1.BASE_PATH,
        uiProfile,
        featureFlags: (0, waba_feature_flags_1.getWabaFeatureFlagsForClient)(),
        deployResilienceEnabled: (0, base_path_1.resolveDeployResilienceForClient)(),
...
function buildDadosResponseCacheKey(ownerEmail, scopeFingerprint, rangeStart, rangeEnd) {
...
app.get("/dados", async (req, res) => {
    try {
        ...
        const authConfigured = (0, waba_auth_service_1.isWabaAuthConfigured)();
        if (authConfigured && !ownerEmail) {
            return res.status(401).json({ error: "Faça login para consultar o dashboard." });
        }
        const scope = authConfigured && ownerEmail
        ...
        const cacheKey = buildDadosResponseCacheKey(ownerEmail, scopeFingerprint, rangeStart, rangeEnd);
        ...
        const applyDadosInstanceScope = (query) => {
            if (scope.globalScope || !scope.filterValues.length)
                return query;
        ...
        if (rangeStart && rangeEnd) {
            ...
            const { count, error: countError } = await applyDadosInstanceScope(supabase
                .from("logs_envios_br")
                .select("id", { count: "exact", head: true })
            ...
            if (typeof totalCount === "number" && totalCount > 0) {
                ...
                while (offset < totalCount && safety < 50) {
                    safety += 1;
                    const { data: senderRows, error: senderErr } = await applyDadosInstanceScope(supabase
                        .from("logs_envios_br")
            ...
            query = applyDadosInstanceScope(query.gte("data_envio_br", startTs).lt("data_envio_br", endTs).limit(5000));
        }
        else {
...
app.post("/integrations/soma/alternativa-campaigns", async (req, res) => {
    try {
        ...
        (0, evo_instance_proxy_service_1.queueSyncProxyBrasilForCampaignSelection)({
        ...
        const campaign = {
            ...
            sentCount: 0,
            ownerEmail,
            pauseReason: "Aguardando ativação. Clique em Ativar campanha para iniciar os disparos.",
            configSnapshot,
        };
...
async function enrichEvoInstanceTagRowsWithLiveState(rows) {
    if (!rows.length)
        return rows;
 ...
 */
async function enrichSelectedCampaignInstancesLive(config, evoRows, opts) {
    const selected = selectedDisparadorNamesFromConfig(config);
    const rows = evoRows.map(cloneEvoInstanceTagRow);
    if (!selected.length)
        return rows;
    const restrictionMap = await (0, whatsapp_connecting_restriction_service_1.getWhatsappConnectingRestrictionMap)();
    const lifecycleMap = await (0, aquecedor_instance_lifecycle_service_1.getAquecedorLifecycleStatusMap)();
    for (const name of selected) {
        ...
        try {
            for (const probe of probeKeys) {
                const detail = await (0, evo_connection_state_service_1.fetchEvoInstanceLiveDetail)(probe, { fresh: true });
                live = detail.state;
                statusReason = detail.statusReason;
                if (String(live || "").trim() || (0, evo_connection_state_service_1.isEvoWhatsAppRestrictedReason)(statusReason)) {
                    probedKey = probe;
                    break;
                }
            }
        ...
        const restrictionActive = probeKeys.some((n) => restrictionMap[n.toLowerCase()]?.active === true);
        const lifecycleRestricted = probeKeys.some((n) => lifecycleMap[n.toLowerCase()]?.phase === "restricted_wait");
        if ((0, evo_connection_state_service_1.isEvoWhatsAppRestrictedReason)(statusReason)) {
            markCampaignInstanceBlocked(key, "statusReason-403");
            if (!restrictionActive)
                markCampaignChipUnsendable(key, "statusReason-403");
        }
        let outboundBroken = false;
        if (opts?.withOutboundHealth &&
            (0, evo_connection_state_service_1.isEvoLiveStateOpen)(live) &&
            !(0, evo_connection_state_service_1.isEvoWhatsAppRestrictedReason)(statusReason)) {
            ...
            if (outboundBroken) {
                ...
                if (!restrictionActive)
                    markCampaignChipUnsendable(key, "outbound-error");
            }
        }
        if (restrictionActive) {
        ...
        const open = (0, evo_connection_state_service_1.campaignChipConnectedForDispatch)({
            liveState: live,
            statusReason,
            outboundBroken,
...
/** Quantos números trocar (1:1 com os vermelhos). Nunca acresce além do teto da campanha. */
function computeCampaignInstancesToAdd(health) {
    return Math.max(0, Number(health.disconnectedCount) || 0);
}
...
function listDisconnectedStoredInstanceNames(selectedNames, evoRows) {
    ...
    for (const name of selectedNames) {
        ...
        const key = String(resolved.instanceKey || stored).trim();
        if ((0, proxy_brasil_config_1.loadProxyBrasilConfig)()?.enabled && key && (0, evo_instance_proxy_service_1.getConfirmedProxyFind)(key) === false) {
            out.push(stored);
        }
    }
    return out;
...
/**
 * Troca 1:1: vermelho sai, substituto entra. Não passa do teto configurado.
 */
function mergeCampaignInstancesReplacingBlocked(input) {
    ...
    const greensN = prevSelected.filter((n) => !disconnected.some((d) => d.toLowerCase() === n.toLowerCase())).length;
    const slotCount = (0, campaign_instance_slots_1.resolveCampaignInstanceSlotCount)(prevSelected, input.slotCount, greensN);
    const sameInstance = (a, b) => {
        const ka = String(resolveStoredNameToEvoTag(a, input.evoRows).instanceKey || a)
            .trim()
            .toLowerCase();
        const kb = String(resolveStoredNameToEvoTag(b, input.evoRows).instanceKey || b)
            .trim()
            .toLowerCase();
        if (ka && kb && ka === kb)
            return true;
        return ka === String(b || "").trim().toLowerCase() || kb === String(a || "").trim().toLowerCase();
    };
    const merged = (0, campaign_instance_slots_1.mergeCampaignSlotsReplacingDisconnected)({
        prevSelected,
        incoming,
        disconnected,
        slotCount,
...
async function persistCampaignSelectedInstances(campaign, selected) {
    ...
    const slotCount = (0, campaign_instance_slots_1.resolveCampaignInstanceSlotCount)(prevSelected.length ? prevSelected : selected, campaign.configSnapshot?.instanceSlotCount, selected.length);
    campaign.configSnapshot = parseDisparosConfig({
        ...(campaign.configSnapshot || DISPAROS_DEFAULTS),
        selectedDisparadorInstances: selected,
        instanceSlotCount: slotCount,
    });
    const supabase = getSupabaseClient();
    if (supabase) {
        try {
...
function campaignSelectionIdentityKeys(selectedNames, evoRows) {
    ...
    for (const raw of selectedNames) {
        ...
        if (key)
            out.add(key);
        if (display && (0, campaign_instance_identity_1.isStableCampaignIdentityToken)(display, key, display)) {
            out.add(display);
        }
...
function campaignInstanceHasProxyBrasil(instanceName) {
    if (!(0, proxy_brasil_config_1.loadProxyBrasilConfig)()?.enabled)
        return true;
    return (0, evo_instance_proxy_service_1.getConfirmedProxyFind)(instanceName) === true;
}
function listConnectedSpareEvoNames(exceptCampaignId, selectedNames, evoRows, maxToAdd = 20) {
    ...
    for (const row of listSpareEvoRowsNotInCampaign(exceptCampaignId, selectedNames, evoRows)) {
        ...
        if (!name)
            continue;
        if (isCampaignInstanceBlocked(name))
            continue;
        if (row.connected !== true)
            continue;
        if (!campaignInstanceHasProxyBrasil(name))
            continue;
        out.push(name);
...
async function resolveLiveSpareEvoNames(exceptCampaignId, selectedNames, evoRows, maxToAdd = 20) {
    const heuristic = listConnectedSpareEvoNames(exceptCampaignId, selectedNames, evoRows, maxToAdd);
    if (heuristic.length)
        return heuristic;
    ...
    for (let i = 0; i < candidates.length && confirmed.length < maxToAdd; i += concurrency) {
        ...
        const states = await Promise.all(chunk.map(async (row) => {
            ...
            if (!name || isCampaignInstanceBlocked(name))
                return { name, open: false };
            try {
                const detail = await (0, evo_connection_state_service_1.fetchEvoInstanceLiveDetail)(name, { fresh: true });
                ...
                if (!String(live || "").trim() &&
                    ...
                    const aliasDetail = await (0, evo_connection_state_service_1.fetchEvoInstanceLiveDetail)(alias, { fresh: true });
                ...
                const cachedOn = (0, evo_instance_proxy_service_1.getConfirmedProxyFind)(name) === true;
                const proxyFind = cachedOn
                    ? true
                    : await (0, evo_instance_proxy_service_1.fetchEvoProxyFindEnabled)(name, callEvoAction, EVO_API_BASE, {
                        timeoutMs: 6000,
                        retries: 0,
                    });
                return { name, open: proxyFind === true };
            }
            catch {
...
function appendIncomingCampaignInstances(prevSelected, incoming, evoRows) {
    ...
    for (const raw of incoming) {
        ...
        if (!key)
            continue;
        const row = evoRows.find((r) => String(r.instanceKey || "").trim().toLowerCase() === key.toLowerCase());
        if (row && evoRowIsInIdentity(row, identity))
            continue;
        if (identity.has(key.toLowerCase()) || identity.has(name.toLowerCase()))
            continue;
        added.push(key);
        selected.push(key);
...
async function deactivateInstancesRemovedFromCampaign(names) {
    for (const raw of names) {
        ...
        try {
            ...
            /* saída da campanha já persistida na seleção */
        }
    }
}
async function persistIncomingCampaignInstances(campaign, incoming, evoRows, opts) {
    const prevSelected = selectedInstanceNamesFromCampaign(campaign);
    ...
    const swapped = opts?.appendOnly === true
        ? { ...appendIncomingCampaignInstances(prevSelected, incoming, evoRows), removedBlocked: [] }
        : mergeCampaignInstancesReplacingBlocked({
            prevSelected,
            incoming,
            evoRows,
            slotCount: campaign.configSnapshot?.instanceSlotCount,
        });
    if (!swapped.added.length && !swapped.removedBlocked.length) {
        return { added: [], removedBlocked: [], selected: prevSelected };
    }
    const selectionUnchanged = swapped.selected.length === prevSelected.length &&
        swapped.selected.every((n, i) => n.toLowerCase() === String(prevSelected[i] || "").toLowerCase());
    ...
    await deactivateInstancesRemovedFromCampaign(swapped.removedBlocked);
    ...
    console.warn(`[Campanha] ${campaign.id}: entram ${swapped.added.join(", ")}${swapped.removedBlocked.length ? ` · saem ${swapped.removedBlocked.join(", ")}` : ""}.`);
    return { added: swapped.added, removedBlocked: swapped.removedBlocked, selected: swapped.selected };
}
...
async function tryAutoSwapDisconnectedCampaignInstances(campaign, evoRows) {
    ...
    const toAdd = health.disconnectedCount;
    ...
    return {
        swapped: result.added.length > 0 || result.removedBlocked.length > 0,
        spareCount: incoming.length,
    };
}
...
function describeCampaignPauseDetail(instanceHealth, options) {
    ...
    if (health?.shouldPauseByDisconnectedRatio === true) {
        parts.push(health.disconnectedPercent +
...
function pauseReasonFromInstanceHealth(health, disconnectedNames) {
...
async function callEvoAction(url, method, body, options) {
    const result = await (0, evo_api_config_1.evoHttpRequestWithBaseFailover)(url, method, {
        apiKey: EVO_API_KEY,
        body,
...
async function callEvoSendTextWithRetry(url, body, maxAttempts = 3) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        const isTransient = (0, evo_send_recovery_service_1.isEvoSendTransientError)(detailStr, last.status);
    ...
    if (last && !last.ok) {
        ...
        const recoveredAccepted = recovered.ok && isEvoSendTextAccepted(recovered.json, recovered.body);
...
function evoChatTextsIncludeMarker(node, marker) {
    return (0, delivery_verify_helpers_1.evoPayloadIncludesNeedle)(node, [marker]);
}
function evoChatTextsIncludeNeedle(node, needles) {
    return (0, delivery_verify_helpers_1.evoPayloadIncludesNeedle)(node, needles);
}
async function probeAquecedorDeliveryViaFindMessages(instanceCandidates, remoteJids, needles, minTimestampMs, fromMe = null) {
    for (const instanceName of instanceCandidates) {
        ...
        for (const remoteJid of remoteJids) {
            const bodies = (0, delivery_verify_helpers_1.buildAquecedorFindMessagesBodies)(remoteJid, fromMe);
            for (const body of bodies) {
                ...
                if (!result.ok)
                    continue;
                if ((0, delivery_verify_helpers_1.evoPayloadIncludesNeedle)(result.json, needles, {
                    minTimestampMs,
                    fromMe,
...
async function probeAquecedorDeliveryViaFindChats(instanceCandidates, needles, minTimestampMs) {
    for (const instanceName of instanceCandidates) {
        const urls = [
...
async function verifyAquecedorMessageDelivered(instanciaDestino, numeroOrigem, messageText, options) {
    ...
    const needleList = (0, delivery_verify_helpers_1.buildAquecedorDeliveryNeedles)(messageText);
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
        ...
        if (!sawDestino) {
            ...
            if (!sawDestino) {
                sawDestino = await probeAquecedorDeliveryViaFindChats(destinoCandidates, needleList, tsFilter);
            }
        }
...
function buildAquecedorSendNumberCandidates(raw) {
    ...
    const out = [];
    const seen = new Set();
    for (const variant of (0, evo_instance_phone_service_1.expandBrazilWhatsAppNumberVariants)(seed)) {
...
async function revertAquecedorPendingAfterFailedSend(supabase, pendingId, options) {
    ...
    await supabase.from("aquecedor").update(payload).eq("id", pendingId);
}
...
const ALTERNATIVA_OPT_OUT_SEED = "😊 Se não quiser mais receber minhas mensagens, é só me avisar, tá bem?";
...
function normalizeAlternativaUrlButtonLabel(raw, fallback = ALTERNATIVA_URL_BUTTON_LABELS[0]) {
    const text = String(raw || "").trim();
    const fold = (s) => s
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "")
...
function newDisparosAiUniqueSeed() {
...
function assembleAlternativaButtonOutbound(generatedText, userCta) {
    ...
    const bodyRaw = parsed
        ...
        : generatedText;
    const body = prepareOutboundWhatsAppText(bodyRaw, { stripUrls: true }) ||
    ...
    const optLine = wrapWhatsAppItalic(optClean) || wrapWhatsAppItalic(ALTERNATIVA_OPT_OUT_SEED);
...
function stripUrlsFromMessageText(message) {
    return String(message || "")
        .replace(/https?:\/\/[^\s)]+/gi, "")
        .replace(/\bwa\.me\/[^\s)]+/gi, "")
        .replace(/^[ \t]*(mais informa[cç][oõ]es|acesse aqui|veja mais|saiba mais|clique aqui)\s*:?[ \t]*$/gim, "")
        .replace(/[ \t]+\n/g, "\n")
        .replace(/\n{3,}/g, "\n\n")
        .replace(/[ \t]{2,}/g, " ")
        .trim();
 ...
 */
function normalizeWhatsAppFormatting(message) {
    let text = String(message || "");
...
function prepareOutboundWhatsAppText(message, opts) {
    let text = String(message || "");
    if (opts?.stripUrls) {
...
async function sendEvoAlternativaUrlButtonMessage(input) {
    ...
    const fullText = prepareOutboundWhatsAppText(String(input.messageText || "").trim(), { stripUrls: true }) ||
        "Olá!";
...
function tryExtractPairingCode(payload) {
    const normalizePairing = (value, keyHint = "") => {
        const raw = String(value || "").trim();
        if (!raw)
            return null;
        const key = String(keyHint || "").toLowerCase();
        if (key === "code" && (raw.includes("@") || raw.includes(",")))
            return null;
        if (/^[A-Z0-9]{4}-[A-Z0-9]{4}$/i.test(raw))
            return raw.toUpperCase();
        if (/^[A-Z0-9]{8}$/i.test(raw)) {
            return `${raw.slice(0, 4).toUpperCase()}-${raw.slice(4).toUpperCase()}`;
        }
        if ((key === "pairingcode" || key === "pairing_code" || key === "pairingCode") &&
...
async function purgeOldEvoSessionsForReconnect(input) {
    ...
    let phone = (0, evo_instance_phone_service_1.normalizeEvoWhatsAppNumber)(String(input.phone || ""));
    if (!phone) {
        ...
        phone = self?.phone ? (0, evo_instance_phone_service_1.normalizeEvoWhatsAppNumber)(self.phone) : "";
    }
    const hits = phone ? (0, evo_reconnect_purge_service_1.collectEvoInstancesSharingPhone)(instances, phone) : [];
    ...
    const { duplicates } = (0, evo_reconnect_purge_service_1.splitCanonicalAndDuplicateNames)(hits, canonicalName);
    for (const extra of duplicates) {
        console.warn(`[Reconnect] ${canonicalName}: apagando clone EVO ${extra} (mesmo número ${phone || "?"}).`);
        await tryDeleteEvoInstance(extra);
        await purgeInstanceLocalState(extra);
        await (0, aquecedor_instance_lifecycle_service_1.removeAquecedorInstanceLifecycle)(extra);
        await (0, whatsapp_connecting_restriction_service_1.clearWhatsappConnectingRestriction)(extra);
        duplicatesDeleted.push(extra);
    }
    let canonicalReset = false;
    if (input.resetCanonical !== false) {
        await (0, whatsapp_connecting_restriction_service_1.clearWhatsappConnectingRestriction)(canonicalName);
        (0, evo_connection_state_service_1.invalidateEvoLiveStateCache)(canonicalName);
...
async function softResetDisconnectedEvoInstanceForQr(instanceName, opts) {
    ...
    try {
        if (campaignProxy) {
            await (0, evo_instance_proxy_service_1.applyProxyBrasilToEvoInstance)(name, callEvoAction, EVO_API_BASE);
        }
        else {
...
function rememberQrRegisterFailure(entry) {
    ...
    if (qrRegisterRecentFailures.length > 20)
        qrRegisterRecentFailures.length = 20;
}
...
async function runRegistrarQrcode(input) {
    ...
    const rememberLifecycleAfterQr = async (createdNew) => {
        await (0, aquecedor_instance_lifecycle_service_1.noteAquecedorInstanceReconnected)(name);
        if (!createdNew)
            return;
    ...
    const inLiveCampaign = (0, proxy_brasil_campaign_rules_1.heldProxyBrasilInstanceNames)(disparosCampaignsMemory.map((c) => ({
        status: c.status,
    ...
    try {
        ...
        console.warn(`[QR] ${name}: falha ao ${keepCampaignProxy ? "aplicar" : "desligar"} proxy antes do QR:`, err);
    }
    ...
    if (isEvoQrRecoverableFailure(qrFetch.lastQrDetail, qrFetch.lastQrStatus)) {
        ...
        let retryPairingFromCreate = null;
        for (const createUrl of createUrls) {
            const createResult = await callEvoAction(createUrl, "POST", createPayload, {
                timeoutMs: Math.min((0, evo_http_client_1.defaultEvoHttpTimeoutMs)(), 30000),
                retries: 2,
...
async function fetchInstanceQrCodeFromEvo(instanceName, number = "", options = {}) {
    ...
    for (let round = 0; round < roundDelaysMs.length; round += 1) {
        ...
        for (const candidate of connectCandidates) {
            ...
            const pairingCode = tryExtractPairingCode(result.json) || tryExtractPairingCode(result.body);
...
app.post("/instancias/:name/qrcode", async (req, res) => {
    try {
        ...
        const campaignProxy = String(req.query.campaignProxy || "").trim() === "1" ||
        ...
        if (campaignProxy) {
            try {
                await (0, evo_instance_proxy_service_1.applyProxyBrasilToEvoInstance)(instanceName, callEvoAction, EVO_API_BASE);
            }
            catch {
                /* */
            }
...
app.post("/instancias/registrar-qrcode", async (req, res) => {
    try {
        ...
        if ((0, waba_auth_service_1.isWabaAuthConfigured)()) {
            ...
            let reserve;
            try {
                reserve = await waba_instance_ownership_service_1.wabaInstanceOwnershipService.claimOnRegister(name, ownerEmail);
            }
            catch (claimError) {
                const detail = claimError instanceof Error
        ...
        void (async () => {
            try {
                ...
                const raw = error instanceof Error
                    ...
                    : String(error);
                const summarized = summarizeEvolutionErrorDetail(error instanceof Error ? error.message : String(error), 0);
                const errorMsg = summarized && summarized.trim()
...
app.post("/instancias/:name/reconnect-purge", async (req, res) => {
    try {
        ...
        return res.json({
            ok: true,
            message: "Sessão antiga e clones do número foram apagados na Evolution. Foguinhos e totais de envio foram mantidos.",
...
app.get("/aquecedor/status", async (req, res) => {
    try {
        ...
        await (0, aquecedor_owner_runtime_registry_1.reloadAquecedorOwnerMotorsFromDisk)();
        const motor = (0, aquecedor_owner_runtime_registry_1.getAquecedorOwnerMotor)(ownerEmail);
        ...
        if (motor.desired === true && summaryAgeMs > 120000) {
            try {
                ...
                (0, aquecedor_owner_runtime_registry_1.updateAquecedorOwnerConnectedSummary)(ownerEmail, connectedActive, resolved.connected);
                void (0, aquecedor_owner_runtime_registry_1.persistAquecedorOwnerSnapshot)(ownerEmail);
            }
            catch (refreshErr) {
                console.warn("[Aquecedor] falha ao refrescar contagem de instâncias no status:", refreshErr instanceof Error ? refreshErr.message : refreshErr);
            }
        }
        const config = await loadAquecedorEffectiveConfig();
        const nowSp = nowInSaoPaulo();
        ...
        const ownerEmail = (0, aquecedor_owner_runtime_registry_1.normalizeAquecedorOwnerEmail)(auth.email);
        return res.json({
            ...(ownerEmail ? (0, aquecedor_owner_runtime_registry_1.buildLiveAquecedorOwnerStatusPayload)(ownerEmail) : {}),
...
app.get("/aquecedor/envios", async (req, res) => {
    ...
    try {
        ...
        if (scopedTechnicalNames.length === 0) {
            const ownerMotor = ownerEmail ? (0, aquecedor_owner_runtime_registry_1.getAquecedorOwnerMotor)(ownerEmail) : null;
        ...
        for (const row of localRows) {
            ...
            if (rowOwner && rowOwner !== ownerEmail)
                continue;
        ...
        if (supabase) {
            ...
            const logsQuery = filterQueueByOwner
                ...
                : supabase
                    ...
                    .order("data_envio_br", { ascending: false })
                    .limit(limit);
            const { data: logsData, error } = (await logsQuery);
...
app.post("/aquecedor/run-once", async (req, res) => {
    ...
    void appendAquecedorCommandLog(lastResult ? `Envio teste: ${lastResult}` : "Envio teste executado.", ownerEmail);
    return res.json({
        ok,
...
app.get("/aquecedor/fila-localizar", async (req, res) => {
    try {
        const auth = (0, waba_request_auth_1.resolveWabaRequestAuth)(req);
        const ownerEmail = (0, aquecedor_owner_runtime_registry_1.normalizeAquecedorOwnerEmail)(auth.email);
        const motor = ownerEmail ? (0, aquecedor_owner_runtime_registry_1.getAquecedorOwnerMotor)(ownerEmail) : null;
        const supabase = getSupabaseClient();
        if (!supabase) {
...
app.get("/aquecedor/diagnostico", async (req, res) => {
    ...
    await (0, aquecedor_owner_runtime_registry_1.reloadAquecedorOwnerMotorsFromDisk)();
    const motor = (0, aquecedor_owner_runtime_registry_1.getAquecedorOwnerMotor)(ownerEmail);
    const persistedStatus = (0, aquecedor_owner_runtime_registry_1.buildAquecedorOwnerStatusPayload)(ownerEmail);
    const diag = {
        runtime: {
            ...motor.runtime,
            ...persistedStatus,
...
app.post("/disparos/gerar-mensagem-ai", async (req, res) => {
    try {
        ...
        const buttonMode = String(req.body?.ctaMode || "").toLowerCase() === "button" ||
            req.body?.buttonMode === true;
        const cta = String(req.body?.cta || config.aiCta || (buttonMode ? "Quero saber mais" : "Responda no link abaixo")).trim();
            ...
            try {
                ...
                if (options.lightLeads) {
                    const light = await (supabase
                        ...
                        .limit(100000));
                    if (!light.error && Array.isArray(light.data))
                        lr = light.data;
                }
                else {
...
async function pickDisparadorInstanceForConfig(config, opts) {
    ...
    for (const name of selectedList) {
        ...
        if (usage && usage.useDisparador === false)
            continue;
        if (isCampaignInstanceBlocked(evoName) || isCampaignInstanceBlocked(name))
            continue;
        const restrictionActive = restrictionMap[evoName.toLowerCase()]?.active === true ||
            restrictionMap[name.toLowerCase()]?.active === true;
        if (restrictionActive)
            continue;
        if (opts?.skipHumanPaused) {
            const life = (await (0, aquecedor_instance_lifecycle_service_1.getAquecedorLifecycleStatusForInstance)(evoName)) ||
                (await (0, aquecedor_instance_lifecycle_service_1.getAquecedorLifecycleStatusForInstance)(name));
        ...
        for (const probe of (0, campaign_instance_identity_1.uniqueProbeNamesForLiveState)(evoName, name)) {
            ...
            live = detail.state;
            statusReason = detail.statusReason;
            if ((0, evo_connection_state_service_1.isEvoLiveStateOpen)(live) && !(0, evo_connection_state_service_1.isEvoWhatsAppRestrictedReason)(statusReason))
                break;
        }
        if (!(0, evo_connection_state_service_1.isEvoLiveStateOpen)(live))
            continue;
        if (!(0, evo_connection_state_service_1.campaignChipConnectedForDispatch)({
            liveState: live,
            statusReason,
            blocked: isCampaignInstanceBlocked(evoName) || isCampaignInstanceBlocked(name),
            outboundBroken: (0, outbound_ack_health_service_1.getCachedAquecedorOutboundHealth)(evoName)?.class === "broken",
        ...
        if (!numero) {
            try {
                numero = String((await (0, evo_instance_phone_service_1.resolveEvoInstancePhone)(evoName)) || "").trim();
            }
            catch {
                numero = "";
            }
...
function generateRandomEvoContactName() {
...
async function saveEvoLeadContactInBackground(instanceName, phone) {
    const instance = String(instanceName || "").trim();
    const number = String(phone || "").replace(/\D/g, "");
    if (!instance || number.length < 10)
        return;
    ...
    for (const url of urls) {
        try {
            ...
            if (result.status !== 404 && result.status !== 405) {
                console.warn(`[Campanha Alternativa] contact/save ${instance} HTTP ${result.status}`);
                return;
            }
        }
        catch (err) {
            console.warn(`[Campanha Alternativa] contact/save ${instance}:`, String(err?.message || err).slice(0, 160));
            return;
        }
    }
}
/** Dispara o save em paralelo: o envio não espera nem atrasa. */
function queueEvoLeadContactSave(instanceName, phone) {
    void saveEvoLeadContactInBackground(instanceName, phone).catch((err) => {
        console.warn("[Campanha Alternativa] contact/save background:", String(err?.message || err).slice(0, 160));
    });
}
async function sendEvoComposingPresenceBeforeText(instanceName, number, typingDelayMs) {
    ...
    if (!sent) {
        console.warn(`[Campanha Alternativa] sendPresence falhou em ${instanceName}; aplicando só delay humano (${delay}ms).`);
    }
...
async function prepareProxyBrasilForCampaignInstancesNow(instanceNames) {
    return (0, evo_instance_proxy_service_1.prepareProxyBrasilSessionsForCampaign)(instanceNames, {
        callEvoAction,
        evoApiBase: EVO_API_BASE,
...
async function releaseHumanPauseForSelectedCampaignInstances(campaign) {
    ...
    for (const name of names) {
        if (isCampaignInstanceBlocked(name))
            continue;
        if (restrictionMap[name.toLowerCase()]?.active === true)
            continue;
...
async function pauseCampaignDueToProxyPrepareFailure(campaignId, reason, options) {
...
function scheduleNextCampaignDispatchDelay(campaignId, config, instanceName) {
...
async function sendCampaignMessengerImageToEvo(input) {
    ...
    if (canInline) {
        ...
        if (result.status === 0) {
            ...
            if (maybeId) {
                console.warn("[Campanha] sendMedia base64 timeout mas messageId presente — trata como enviado (sem fallback URL):", maybeId);
                return { ok: true, json: result.json, status: 0, body: String(result.body || "timeout") };
            }
            console.warn("[Campanha] sendMedia base64 timeout/sem HTTP — não faz fallback URL (evita 2 imagens):", String(result.body || "").slice(0, 160));
...
async function processOneCampaignDispatch(campaignId) {
    ...
    if (Date.now() < nextAt)
        return;
    if (campaignDispatchBusy.has(campaignId))
        return;
    ...
    try {
        ...
        if (proxyCfg?.enabled) {
            const proxyFind = await (0, evo_instance_proxy_service_1.fetchEvoProxyFindEnabled)(instancePick.instancia, callEvoAction, EVO_API_BASE, { timeoutMs: 6000, retries: 0 });
        ...
        if (!(0, evo_connection_state_service_1.isEvoLiveStateOpen)(instanceLiveState)) {
            ...
            const conn = (0, proxy_brasil_campaign_rules_1.classifyProxyBrasilConnection)(instanceLiveState);
            await pauseCampaignDueToProxyPrepareFailure(campaignId, `Instância ${instancePick.instancia} saiu de open durante o disparo (${instanceLiveState || "desconhecido"}). Reconecte no Aquecedor e ative de novo.`, { instanceName: instancePick.instancia, disableProxy: conn === "disconnected" });
            lead.status = "pending";
            return;
        }
...
async function stopAllDispatchActivityOnServer(ownerEmail) {
    const normalizedOwner = (0, aquecedor_owner_runtime_registry_1.normalizeAquecedorOwnerEmail)(ownerEmail);
    if (normalizedOwner) {
...
function countCampaignLeadsSent(campaignId, sentFallback) {
...
app.post("/disparos/campanhas", (req, res, next) => {
    ...
    try {
        ...
        if (toDisableProxy.length) {
            (0, evo_instance_proxy_service_1.queueDisableProxyBrasilOnInstances)(toDisableProxy, callEvoAction, EVO_API_BASE);
        }
        const proxyPrepareResults = (0, proxy_brasil_config_1.loadProxyBrasilConfig)()?.enabled
        ...
        return res.json({
            ok: true,
            message: (proxyPrepareNotReady.length
...
app.get("/disparos/campanhas", async (req, res) => {
    try {
        const buildCampaignRuntimeStage = (item, configSnapshot, nowSp, instanceHealth, disconnectedNames) => {
        ...
        const spareByCampaignId = new Map();
...
app.post("/disparos/campanhas/:id/instancias", async (req, res) => {
    try {
        ...
        const instancesToAdd = healthBefore.disconnectedCount;
        const disconnectedNames = listDisconnectedStoredInstanceNames(selectedNames, evoRowsAll);
        let incoming = [];
        if (auto) {
            if (!disconnectedNames.length) {
                return res.status(409).json({
                    error: "Não há instância desconectada para substituir. A campanha permanece com " +
            ...
            const swapNote = removedCount > 0
                ? ` ${incoming.length ? "Substituímos" : "Saiu da campanha"} ${removedCount} número(s) bloqueado(s)/offline (${swapped.removedBlocked.join(", ")}).`
                : "";
            return res.json({
                ok: true,
                id,
...
(0, waba_leads_cnpj_routes_1.registerWabaLeadsCnpjRoutes)(app);
...
(0, waba_device_cloud_routes_1.registerDeviceCloudRoutes)(app);
...
const httpServer = app.listen(PORT, () => {
    ...
    void (async () => {
        ...
        setInterval(() => {
            (0, whatsapp_connecting_restriction_service_1.recheckWhatsappConnectingRestrictions)()
                ...
                if (result.cleared.length) {
                    console.log(`[WA-Restrição] liberada(s) após recheck 60min: ${result.cleared.join(", ")}`);
                }
            })
        ...
        if (ENABLE_BACKGROUND_PROCESSING && !MAINTENANCE_MODE) {
            ...
            setInterval(() => {
                if (campaignDispatchTickRunning)
                    return;
