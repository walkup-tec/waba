...
function classifySystemLogMotivo(input) {
    ...
    if (/entrypoint|websecure|\bweb\b.*https|main\.ya?ml|yaml|rota(dor)?|host\(`|backend url|30211|30210/.test(detail)) {
        return "Yaml";
    }
    if (/docker|swarm|container|0\/1|replicas|service update|easypanel-traefik|overlay|docker-proxy/.test(detail)) {
        return "Docker";
    }
    if (/traefik|bad gateway|502|000|tls|schannel|certificate|acme|:443|gateway|proxy/.test(detail) ||
        ...
        if (/(timeout|econnrefused|enotfound|network|unreachable|could not resolve|abort)/.test(detail) &&
            downCount >= 2) {
            return "Servidor";
        }
        return "Traefik";
