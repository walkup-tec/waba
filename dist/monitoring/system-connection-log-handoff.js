...
exports.buildSystemLogResumo = buildSystemLogResumo;
exports.buildSystemLogHandoffBrief = buildSystemLogHandoffBrief;
...
function classifyHint(motivo, probeDetail) {
...
function buildSystemLogResumo(input) {
    const status = input.status === "conexao" ? "UP" : "DOWN";
    const dur = input.status === "conexao" && input.durationHours != null
 ...
 */
function buildSystemLogHandoffBrief(input) {
    const playbook = (0, uptime_playbooks_1.resolveUptimePlaybookForTarget)(input.targetKey);
    const peers = input.peersDown && input.peersDown.length
    ...
    lines.push("", "### Stack / ambiente", `- VPS: ${VPS_HINT}`, "- Backends canônicos: host gateway `http://172.17.0.1:<porta>/` (30180 WABA, 30210 disparos, 30211 bet, 30181 EVO)", "- Evitar `docker service update --force easypanel-traefik` / HUP se Traefik já estiver 1/1 e :443 up", "- Script heal login: `/root/waba-infra/heal-waba-login-vps.sh run`");
    if (playbook) {
        lines.push("", "### Playbook do alvo", `- ${playbook.label}: ${playbook.summary}`, "", "### Comandos sugeridos (root no VPS)", commands || "(sem passos cadastrados)");
    }
    else {
        lines.push("", "### Comandos genéricos sugeridos", "1. `curl -sS --max-time 10 http://127.0.0.1:30180/health`", "2. `ss -tlnp | grep -E ':443|:30180'`", "3. `docker service ls | grep -E 'traefik|waba_disparador'`", "4. `bash /root/restore-easypanel-traefik-backends-vps.sh`", "5. `bash /root/waba-infra/heal-waba-login-vps.sh run`");
    }
    lines.push("", "### Pedido ao desenvolvedor / agente", "Diagnosticar a causa da falha de conectividade, aplicar correção mínima (publish/backends/YAML/Docker) e validar:", `1) probe local do backend; 2) HTTPS do alvo \`${input.targetLabel}\` → HTTP 200; 3) não derrubar :443 sem necessidade.`, "", "### Resumo uma linha", buildSystemLogResumo(input));
    return lines.join("\n");
}
