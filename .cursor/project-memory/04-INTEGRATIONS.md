...
- Inbox por chip: `POST /integrations/meta/whatsapp/phone-numbers/inbox` (`enabled: true` + telefone/nome). Ligar tenta `POST /{WABA}/subscribed_apps`. Lista: `GET /integrations/meta/whatsapp/inbox/conversations` com `channels[]` (nome + `displayPhoneNumber`). Envio Cloud e compositor usam o `phone_number_id` do chip ligado.
