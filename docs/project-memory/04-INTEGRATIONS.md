...
- Cabeçalho IMAGE: upload resumable Graph (`POST /{app-id}/uploads` + binário `upload:{session}`). MIME pelo magic dos bytes; `file_name` só ASCII (`header.png`/`header.jpg`). Sem teto local de 5 MB; o HTTP do upload aceita até 16 MB. A Graph ainda pode recusar arquivo grande. Doc: https://developers.facebook.com/docs/graph-api/guides/upload
