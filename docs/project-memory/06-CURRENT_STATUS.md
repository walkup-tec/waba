...
- Disparo Cloud na aba Templates: planilha com telefones em qualquer formato comum no Brasil; a prévia mostra o E.164 que a Meta recebe. Envio só de template aprovado, pelo número Ativo e disponível. O campo de template tem filtro de categoria (primeira opção todas) e o rótulo é `nome-categoria`. Ao usar o número ele fica ocupado até a campanha finalizar e o relatório ser gerado. O disparo pede a campanha do assinante atendida no Laboratório; o relatório dessa campanha fecha com dados da Meta e cliques. Campanhas de operadores sem Laboratório permanecem com relatório manual.
