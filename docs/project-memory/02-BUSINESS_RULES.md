...
- Disparo Cloud (aba Templates): Portfólio → template **APPROVED** → número **Ativo e disponível** → campanha do assinante atendida no Laboratório → planilha. No template há filtro de categoria (primeira opção **todas**). O select exibe só `nome-categoria` (ex.: `consulta_margem_atualizacao_1-Utilidade`). A coluna de telefone é obrigatória; Nome/Número só se o BODY do template tiver essa variável. Destinos são normalizados para E.164 da Meta (sem `+`). Duplicata na mesma campanha (com/sem 9º dígito) é ignorada. Clique no botão URL conta no disparo via `/s/:slug` e só entra no relatório dessa campanha do Laboratório.
