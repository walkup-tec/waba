...
- Na tabela de templates: **Visualizar** abre o modal **Seu modelo**, com a imagem/vídeo de cabeçalho quando existir. **Excluir** apaga na Graph (`DELETE /{waba}/message_templates` com `hsm_id`+`name`) e no banco do tenant. Nome aprovado fica bloqueado 30 dias na Meta. Não há painel nem botão de envio de teste.
