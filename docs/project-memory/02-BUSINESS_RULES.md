...
- Disparo Cloud (aba Templates): Portfólio → template **APPROVED** → número **Ativo e disponível** → campanha do assinante **Em andamento** atendida no Laboratório → planilha. No template há filtro de categoria (primeira opção **todas**). O select exibe só `nome-categoria` (ex.: `consulta_margem_atualizacao_1-Utilidade`). A campanha aparece como `nome do assinante - campanha - envios`. A coluna de telefone é obrigatória; Nome/Número só se o BODY do template tiver essa variável. Destinos são normalizados para E.164 da Meta (sem `+`). Duplicata na mesma campanha (com/sem 9º dígito) é ignorada. Clique no botão URL conta no disparo via `/s/:slug` e só entra no relatório dessa campanha do Laboratório.
...
- Relatório do assinante: se o operacional atribuído **não** tem Laboratório, o preenchimento continua manual (enviados/entregues/lidos/falhados) e **não há cliques**. Se o atendente tem Laboratório, os indicadores vêm da Meta e o relatório inclui cliques. O plano oficial/alternativa do assinante não muda essa regra.
