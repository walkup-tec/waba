 ...
 * Grava manualActiveOverride + corrige createdAt no cache EVO (evita revert no refresh).
 *
 * Uso no container:
...
let evo = readJson(evoPath, { items: [] });
if (!Array.isArray(evo.items)) evo.items = [];
let evoPatched = false;
...
if (!evoPatched) {
  evo.items.push({ name: instanceArg, createdAt: grandfatherCreatedAt });
  evoPatched = true;
}
writeJsonAtomic(evoPath, evo);

try {
  const lifecycleMod = require(path.join(__dirname, "..", "dist", "services", "aquecedor-instance-lifecycle.service.js"));
  if (typeof lifecycleMod.invalidateAquecedorLifecycleCache === "function") {
    lifecycleMod.invalidateAquecedorLifecycleCache();
  }
} catch {
  /* processo separado ou dist ausente */
}
  ...
  JSON.stringify(
    {
      ...
      phase: "active",
      manualActiveOverride: true,
      evoCreatedAtPatched: evoPatched,
      ownerUpdated,
      ownerEmail: ownerUpdated ? ownerEmail : null,
      note: "Reinicie o container (docker restart) se a UI ainda mostrar Preparando após este script.",
    },
    null,
    2,
  ),
