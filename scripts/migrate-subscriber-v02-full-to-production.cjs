/**
 * Migração completa de um assinante V02 → produção (/app/data).
 *
 * Exportar no PC (dados V02 locais):
 *   set WABA_V02_DATA_DIR=D:\Waba\data\v02
 *   node scripts/migrate-subscriber-v02-full-to-production.cjs mozart.pmo@gmail.com --export-dir C:\Temp\mozart-v02-export
 *
 * Aplicar no container (após docker cp da pasta export):
 *   node scripts/migrate-subscriber-v02-full-to-production.cjs mozart.pmo@gmail.com --import-dir /tmp/mozart-v02-export --apply-data-dir /app/data
 */
...
if (!emailArg.includes("@")) {
  console.error(
    "Uso: node scripts/migrate-subscriber-v02-full-to-production.cjs email@dominio.com [--export-dir DIR] [--import-dir DIR --apply-data-dir /app/data] [--print-bundle]",
  );
  process.exit(1);
}
...
function collectCampaignIntakeAssets(v02Dir, email) {
  ...
  const intakes = (store.intakes || []).filter(
...
function buildFullBundle(email, v02Dir) {
  const subscribers = readJson(path.join(v02Dir, "waba-subscribers.json"), { version: 1, subscribers: [] });
  ...
  const ticketsStore = readJson(path.join(v02Dir, "waba-support-tickets.json"), {
    version: 1,
    tickets: [],
  });
  const supportTickets = (ticketsStore.tickets || []).filter(
    (row) => normalizeEmail(row?.ownerEmail) === email,
  );
  ...
  const financeiroSettlements = (settlementsStore.settlements || []).filter(
    (row) => normalizeEmail(row?.ownerEmail) === email,
  );

  const enviosLog = readJson(path.join(v02Dir, "aquecedor-envios-log.json"), []);
  const aquecedorEnviosLog = Array.isArray(enviosLog)
    ? enviosLog.filter((row) => normalizeEmail(row?.ownerEmail) === email)
    : [];
...
function applyFullBundle(bundle, dataDir, importRoot = "") {
  ...
  const subIdx = store.subscribers.findIndex((row) => normalizeEmail(row?.email) === email);
  if (subIdx >= 0) {
    store.subscribers[subIdx] = { ...store.subscribers[subIdx], ...payload, id: store.subscribers[subIdx].id };
  ...
  const beforeCampaigns = dispatchStore.campaigns.length;
  dispatchStore.campaigns = mergeById(dispatchStore.campaigns, bundle.campaigns || []);
  dispatchStore.savedAt = new Date().toISOString();
  summary.campaignsMerged = dispatchStore.campaigns.length - beforeCampaigns + (bundle.campaigns || []).length;
  ...
  const intakesToApply = (bundle.campaignIntakes || []).map((row) =>
    rewriteIntakePathsForDataDir(row, dataDir),
  );
  ...
  for (const asset of bundle.campaignIntakeAssets || []) {
    ...
    const rel = path.join("campaign-intakes", intakeId, fileName);
    const candidates = [
  ...
  if (!Array.isArray(ticketsStore.tickets)) ticketsStore.tickets = [];
  const beforeTickets = ticketsStore.tickets.length;
  ticketsStore.tickets = mergeById(ticketsStore.tickets, bundle.supportTickets || []);
  summary.supportTicketsMerged = ticketsStore.tickets.length - beforeTickets + (bundle.supportTickets || []).length;
  ...
  if ((bundle.aquecedorEnviosLog || []).length > 0) {
    ...
    const logStore = readJson(logPath, []);
    const logList = Array.isArray(logStore) ? logStore : [];
    const merged = mergeById(logList, bundle.aquecedorEnviosLog);
    writeJsonAtomic(logPath, merged);
    summary.aquecedorEnviosLogMerged = bundle.aquecedorEnviosLog.length;
  }
...
function exportBundle(bundle, outDir) {
  ...
  for (const asset of assets) {
    ...
    bundleForDisk.campaignIntakeAssets.push({
      intakeId,
      fileName,
      sourcePath: dest,
    });
...
function main() {
  if (exportDir) {
    const bundle = buildFullBundle(emailArg, V02_DIR);
  ...
  if (importDir && applyDataDir) {
    ...
    const result = applyFullBundle(bundle, path.resolve(applyDataDir), path.resolve(importDir));
    console.log(JSON.stringify(result, null, 2));
    return;
  }

  console.error("Informe --export-dir (PC) ou --import-dir + --apply-data-dir (servidor).");
  process.exit(1);
}
