...
RUN echo ">>> [builder] npm ci" \
  && npm ci --no-audit --no-fund \
  && echo ">>> [builder] npm ci OK"
...
RUN echo ">>> [builder] npm run build" \
  && npm run build \
  && echo ">>> [builder] npm run build OK"

RUN echo ">>> [builder] npm prune --omit=dev" \
  && npm prune --omit=dev \
  && echo ">>> [builder] npm prune OK"
