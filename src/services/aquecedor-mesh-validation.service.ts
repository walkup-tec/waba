...
const pendingByMarker = new Map<string, MeshPendingPair>();
...
function buildComparableDigitKeys(raw: string): Set<string> {
...
function extractRemoteJid(node: Record<string, unknown>): string {
  ...
  for (const candidate of candidates) {
    ...
    if (text && !text.includes("@g.us")) return text;
  }
  return "";
}

function remoteJidMatchesOrigem(remoteJid: string, origDigitKeys: Set<string>): boolean {
  const jidDigits = String(remoteJid || "").split("@")[0].replace(/\D/g, "");
  if (!jidDigits) return false;
  if (origDigitKeys.has(jidDigits)) return true;
  const comparable = buildComparableDigitKeys(jidDigits);
  for (const key of comparable) {
    if (origDigitKeys.has(key)) return true;
  }
  return false;
}

function payloadIncludesMeshMarker(node: unknown, marker: string, depth = 0): boolean {
  ...
  for (const value of Object.values(obj)) {
    if (value && typeof value === "object") {
      if (payloadIncludesMeshMarker(value, marker, depth + 1)) return true;
    }
  }
  return false;
}

export function beginAquecedorMeshSession(): void {
  meshSessionActive = true;
  pendingByMarker.clear();
}

export function endAquecedorMeshSession(): void {
  meshSessionActive = false;
  pendingByMarker.clear();
}

export function registerAquecedorMeshPending(input: {
  marker: string;
  destInstance: string;
  origInstance: string;
  origDigits: string;
...
export function isAquecedorMeshPairConfirmed(marker: string): boolean {
  const key = String(marker || "").trim().toLowerCase();
  const row = pendingByMarker.get(key);
  return Boolean(row?.confirmedAtMs);
}

export function getAquecedorMeshConfirmDetail(marker: string): string {
  const key = String(marker || "").trim().toLowerCase();
  return pendingByMarker.get(key)?.confirmDetail || "";
}

export function handleAquecedorMeshWebhook(body: unknown): void {
  if (!meshSessionActive || !body || typeof body !== "object") return;
  const payload = body as Record<string, unknown>;
  const event = String(payload.event || "").toUpperCase();
  if (event && event !== "MESSAGES_UPSERT" && event !== "MESSAGES.UPSERT") return;

  const instanceName = String(
    payload.instance || payload.instanceName || payload.sender || "",
  ).trim();
  const instanceKey = normalizeInstanceKey(instanceName);

  for (const row of pendingByMarker.values()) {
    if (row.confirmedAtMs) continue;
    if (instanceKey && normalizeInstanceKey(row.destInstance) !== instanceKey) continue;
    if (!payloadIncludesMeshMarker(payload, row.marker)) continue;

    const fromMe = extractFromMe(payload);
    if (fromMe === true) continue;

    const remoteJid = extractRemoteJid(payload);
    if (remoteJid && !remoteJidMatchesOrigem(remoteJid, row.origDigitKeys)) continue;
