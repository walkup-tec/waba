...
import {
  ...
  type CloudApiRecipientErr,
  type CloudApiRecipientOk,
 ...
 */
export function spreadsheetPhoneCellDigits(raw: unknown): string {
  let text = "";
  ...
  if (digits.startsWith("0") && digits.length >= 11 && digits.length <= 14) {
    ...
    if (national || intlBr) digits = withoutTrunk;
  }
  return digits;
}

function pickBrazilMobileWaId(digits: string): string {
  const variants = expandBrazilWhatsAppNumberVariants(digits);
  const mobileIntl = variants.find(
    (item) => item.startsWith("55") && item.length === 13 && item.charAt(4) === "9",
  );
  if (mobileIntl) return mobileIntl;
  const landIntl = variants.find((item) => item.startsWith("55") && item.length === 12);
  if (landIntl) return landIntl;
  const mobileNational = variants.find(
    (item) => !item.startsWith("55") && item.length === 11 && item.charAt(2) === "9",
  );
  if (mobileNational) return `55${mobileNational}`;
 ...
 */
export function normalizeMetaSpreadsheetRecipient(
  raw: unknown,
): CloudApiRecipientOk | CloudApiRecipientErr {
  ...
  if (digits.length >= 12 && digits.length <= 15 && !digits.startsWith("55")) {
    return normalizeCloudApiRecipient(digits);
  }
  return normalizeCloudApiRecipient(pickBrazilMobileWaId(digits));
}

/** Chave de dedupe na mesma campanha (BR sem o 9 móvel quando aplicável). */
export function metaSpreadsheetRecipientDedupeKey(waId: string): string {
  return canonicalizeBrazilWhatsAppNumber(waId) || waId;
}
