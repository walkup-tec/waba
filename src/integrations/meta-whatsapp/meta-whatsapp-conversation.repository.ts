...
export class MetaWhatsappConversationRepository {
  ...
  async findByTenantPhoneContact(
    tenantId: string,
  ...
  async upsertForContact(
    ...
    const existing = phoneNumberId
      ? await this.findByTenantPhoneContact(
          input.tenantId,
          phoneNumberId,
          input.contactWaId,
        )
  ...
  async listForInbox(input: {
    ...
    const connectionIds = (input.connectionIds || [])
      ...
      .filter(Boolean);
    if (connectionIds.length) query = query.in("connection_id", connectionIds);
  ...
  async listUnreadByPhone(
    ...
    if (ids.length) query = query.in("connection_id", ids);
    const { data, error } = await query;
    if (error) throw new Error(error.message);
    return (data || []).map((row) => {
      const rec = asRow(row);
