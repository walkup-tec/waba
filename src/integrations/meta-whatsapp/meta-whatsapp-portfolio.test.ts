...
describe("meta portfolio service", () => {
  ...
  it("aplica a foto mesmo se a Meta recusar o novo nome de exibição", async () => {
    const png =
      "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8BQDwAEhQGAhKmMIQAAAABJRU5ErkJggg==";
    const posts: Array<{ path: string; body?: Record<string, unknown>; query?: Record<string, string> }> = [];
    const graph = async (input: {
      path: string;
      method: string;
      query?: Record<string, string>;
      ...
      if (input.method === "POST") {
        posts.push({ path: input.path, body: input.body, query: input.query });
        if (input.path === "phone-1" && input.query?.new_display_name) {
          return {
            ok: false,
            status: 400,
            graphCode: "100",
            json: { error: { message: "Invalid display name", code: 100 } },
          };
        }
        return { ok: true, status: 200, json: { success: true } };
      }
    ...
    const service = new MetaWhatsappConnectionService(
      ...
      graph as any,
      decryptMetaToken,
      async () => ({ handle: "HANDLE_PIC_PARTIAL" }),
    );
    const updated = await service.updatePhoneProfileFromAuth(auth, {
      phoneNumberId: "phone-1",
      displayName: "Nesio",
    ...
    const profilePost = posts.find((item) => item.path === "phone-1/whatsapp_business_profile");
    assert.equal(profilePost?.body?.profile_picture_handle, "HANDLE_PIC_PARTIAL");
    assert.equal(updated.photoUpdated, true);
  ...
  it("BM administrado sem conexão: WABA owned e chips entram no card", async () => {
    const walkup = {
      ...connectedRow(),
      id: "conn-walkup-sander",
      metaBusinessId: "4141369862822598",
      wabaId: "1014470201624992",
      accessTokenEncrypted: encryptMetaToken("token-walkup"),
    };
    ...
    const graph = async (input: { path: string }) => {
              ...
              data: [
                {
                  ...
                  phone_numbers: { data: [sanderPhones[0]] },
                },
                {
                  id: "waba-sander-2",
                  name: "Sander roosevelt de souza",
                  phone_numbers: { data: [sanderPhones[1]] },
                },
              ],
            },
      ...
      if (input.path === "1588459689692010/owned_whatsapp_business_accounts") {
        return {
          ...
          json: {
            data: [
              { id: "waba-sander-1", name: "Sander de souza" },
              { id: "waba-sander-2", name: "Sander roosevelt de souza" },
            ],
          },
        };
      }
      if (input.path === "waba-sander-1/phone_numbers") {
        return { ok: true, status: 200, json: { data: [sanderPhones[0]] } };
      }
      if (input.path === "waba-sander-2/phone_numbers") {
        return { ok: true, status: 200, json: { data: [sanderPhones[1]] } };
      }
      return { ok: true, status: 200, json: { data: [] } };
    };
    const service = new MetaWhatsappConnectionService(
    ...
    assert.ok(numbers.some((item) => String(item.phoneNumberId || "") === "phone-sander-2"));
    const walkupCard = (assets.portfolios || []).find((item) => item.id === "4141369862822598");
    assert.equal(
      (walkupCard?.numbers || []).some((item) => String(item.phoneNumberId || "").startsWith("phone-sander")),
      false,
    );
  });

  it("BM sem owned: WABA client da agência cujo dono é esse BM entra no card", async () => {
    const walkup = {
      ...connectedRow(),
      id: "conn-walkup-partner",
      metaBusinessId: "4141369862822598",
      wabaId: "1014470201624992",
      accessTokenEncrypted: encryptMetaToken("token-walkup"),
    };
    const sanderPhone = {
      id: "phone-sander-partner",
      display_phone_number: "+1 555-425-7122",
      verified_name: "Sander roosevelt de souza",
      status: "CONNECTED",
      code_verification_status: "VERIFIED",
    ...
    const graph = async (input: { path: string }) => {
      ...
      if (input.path === "waba-sander-partner/phone_numbers") {
    ...
    const assets = await service.listPortfolioAssets(auth);
    const sander = (assets.portfolios || []).find((item) => item.id === "1588459689692010");
    assert.equal(sander?.wabaId, "waba-sander-partner");
    const numbers = sander?.numbers || [];
    assert.equal(numbers.length, 1);
    assert.equal(numbers[0]?.phoneNumberId, "phone-sander-partner");
    const walkupCard = (assets.portfolios || []).find((item) => item.id === "4141369862822598");
    assert.equal(
  ...
  it("busca o BM da Flaviane com o token já conectado quando me/businesses omite", async () => {
    const walkup = {
      ...connectedRow(),
      id: "conn-walkup",
      metaBusinessId: "4141369862822598",
  ...
  it("pagina phone_numbers da Graph e une todas as páginas", async () => {
    ...
    const assets = await service.listPortfolioAssets(auth);
    assert.ok(phoneCalls.length >= 2);
    assert.ok(phoneCalls.some((query) => !query?.after));
    assert.ok(phoneCalls.some((query) => query?.after === "cursor-page-2"));
  ...
  it("une chip Pendente omitido quando health_status ou tier só devolve CONNECTED", async () => {
    const andre = {
      ...connectedRow(),
      id: "conn-andre-omit",
      metaBusinessId: "1759044748332124",
    ...
    const graph = async (input: { path: string; query?: Record<string, string> }) => {
      ...
      if (input.path === "1759044748332124/owned_whatsapp_business_accounts") {
        return { ok: true, status: 200, json: { data: [{ id: "2458602464640240" }] } };
      }
      return { ok: true, status: 200, json: { data: [] } };
    };
    const service = new MetaWhatsappConnectionService(
