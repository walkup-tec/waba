...
import {
  ...
  parseMetaTemplateAiHasLinkButton,
...
export class MetaWhatsappTemplateAiService {
  ...
  async generateFromAuth(
    ...
    const hasLinkButton = parseMetaTemplateAiHasLinkButton(input);
    ...
    try {
      ai = await this.openAi({
        instructions: buildMetaTemplateAiInstructions({ hasLinkButton }),
    ...
    let result;
    try {
      result = shapeMetaUtilityAiOutput(validateMetaTemplateAiOutput(ai.value), variableType, hasLinkButton);
  ...
  async uploadHeaderMediaFromAuth(
    ...
    if (isMetaGraphUploadCooldown()) {
      const failed = new MetaWhatsappError("template_upload_failed");
      failed.message = metaGraphUploadCooldownMessage();
      throw failed;
    }
    ...
    for (const candidate of candidates) {
      ...
      try {
        ...
        if (isHeaderUploadAppRateLimit(error)) {
          markMetaGraphUploadCooldown();
          break;
        }
      }
