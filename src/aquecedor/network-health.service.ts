import type {
  NetworkHealthPairRow,
  NetworkHealthPhoneRow,
  NetworkHealthReport,
  OwnerConversationGraph,
  RelationshipMatrix,
} from "./conversation-pair.types";
import { buildConversationPairKey, dayKeySaoPaulo } from "./conversation-graph.service";

function toPhoneRows(owner: OwnerConversationGraph): NetworkHealthPhoneRow[] {
  const today = dayKeySaoPaulo();
  const rows: NetworkHealthPhoneRow[] = [];
...
function toPairRows(owner: OwnerConversationGraph): NetworkHealthPairRow[] {
  const today = dayKeySaoPaulo();
  const rows: NetworkHealthPairRow[] = Object.entries(owner.pairs).map(([pairKey, pair]) => ({
    pairKey,
    a: pair.a,
    b: pair.b,
    sentAB: pair.sentAB,
...
function buildRelationshipMatrix(
  ...
  const cells: Array<Array<number | null>> = sorted.map(() =>
    sorted.map(() => null as number | null),
  );
  for (let i = 0; i < sorted.length; i += 1) {
    for (let j = 0; j < sorted.length; j += 1) {
      ...
      const key = buildConversationPairKey(sorted[i], sorted[j]);
...
export function buildNetworkHealthReport(
  ownerEmail: string,
  owner: OwnerConversationGraph,
  options: { instanceNames?: string[] } = {},
): NetworkHealthReport {
  const phones = toPhoneRows(owner);
  const pairs = toPairRows(owner);
  const labels =
    options.instanceNames?.length
  ...
  const reciprocityPercent =
    totalMessages > 0
      ...
      : 100;
  const avgSentToday =
    phones.length > 0 ? phones.reduce((s, p) => s + p.sentToday, 0) / phones.length : 0;
  const avgReceivedToday =
    phones.length > 0 ? phones.reduce((s, p) => s + p.receivedToday, 0) / phones.length : 0;
  ...
  const mostUsedPairs = [...pairs].sort(
    (a, b) => b.totalMessages - a.totalMessages || a.pairKey.localeCompare(b.pairKey),
  );
  const leastUsedPairs = [...pairs].sort(
    (a, b) => a.totalMessages - b.totalMessages || a.pairKey.localeCompare(b.pairKey),
  );
