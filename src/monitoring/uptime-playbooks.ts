import type { VpsCpuPlaybook, VpsCpuPlaybookStep } from "../infra/vps-cpu-monitor.types";

export type UptimePlaybookTargetKey =
  | "site_drax"
  | "site_bet"
  | "site_disparos"
  | "app_waba"
...
const landingSyncSteps: VpsCpuPlaybookStep[] = [
  {
    order: 1,
...
const appWabaSteps: VpsCpuPlaybookStep[] = [
  {
    order: 1,
...
const siteDraxSteps: VpsCpuPlaybookStep[] = [
  {
    order: 1,
...
const evoSteps: VpsCpuPlaybookStep[] = [
  {
    order: 1,
...
const asaasSteps: VpsCpuPlaybookStep[] = [
  {
    order: 1,
...
const playbooks: Record<UptimePlaybookTargetKey, VpsCpuPlaybook> = {
  site_bet: {
    id: "uptime-site-bet",
    label: "bet.waba.info",
...
export const UPTIME_PLAYBOOK_TARGET_KEYS = Object.keys(playbooks) as UptimePlaybookTargetKey[];

export const isKnownUptimePlaybookTargetKey = (key: string): key is UptimePlaybookTargetKey =>
  UPTIME_PLAYBOOK_TARGET_KEYS.includes(key as UptimePlaybookTargetKey);

export const resolveUptimePlaybookForTarget = (targetKey: string): VpsCpuPlaybook | null => {
  if (!isKnownUptimePlaybookTargetKey(targetKey)) return null;
