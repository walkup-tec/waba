...
export const registerWabaOperacionalCampanhasRoutes = (app: Express) => {
  ...
  app.get("/admin/operacional/campanhas/:id/operacionais-transferencia", (req, res) => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Não foi possível listar operacionais.";
  ...
  app.post("/admin/operacional/campanhas/:id/bm-inoperante", async (req, res) => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Não foi possível reatribuir a campanha.";
      const status = /não encontrada|não disponível|BM inoperante|Somente campanhas|atribuída|fornecedor/i.test(
        message,
      )
        ? 400
        : 500;
  ...
  app.post("/admin/operacional/campanhas/:id/atribuir", async (req, res) => {
    ...
    try {
      ...
      const campaign = await operacionalCampanhasService.assignCampaignToOperacional(
        req.params.id,
        String(body.operacionalEmail ?? body.email ?? ""),
      ...
      const message =
        error instanceof Error ? error.message : "Não foi possível atribuir a campanha.";
      const status = /não encontrada|não disponível|Somente|Informe|Operacional|segmento|API/i.test(
        message,
      )
        ? 400
        : 500;
  ...
  app.post("/admin/operacional/campanhas/:id/reenviar-email-operacional", async (req, res) => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Não foi possível reenviar o e-mail operacional.";
  ...
  app.post("/admin/operacional/campanhas/:id/reportar-erro", (req, res) => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Não foi possível reportar o erro.";
      const status = /não encontrada|não disponível|Somente campanhas|já foi finalizada|foi cancelada|justificativa|não foi possível registrar/i.test(
        message,
      )
        ? 400
        : 500;
