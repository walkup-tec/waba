import { isAsaasConfigured } from "../billing/asaas.client";
...
const buildDayKeys = (days: number): string[] => {
  const keys: string[] = [];
  const now = new Date();
...
const resolveEventDayKey = (iso: string): string => {
  const value = String(iso || "").trim();
  if (!value) return "";
  return value.slice(0, 10);
};

const buildSubscriberRevenueGrowth = (
  subscribers: Array<{ createdAt: string }>,
  orders: ReturnType<WabaBillingOrderRepository["list"]>,
) => {
  ...
  for (const subscriber of subscribers) {
    ...
    if (!dailyNewSubscribers.has(key)) continue;
  ...
  const series = dayKeys.map((date) => {
    ...
    const revenuePerUserCents =
      cumulativeSubscribers > 0 ? Math.round(cumulativeRevenueCents / cumulativeSubscribers) : 0;
    return {
      date,
  ...
  const revenuePerUserAtPeriodStartCents =
  ...
  const revenuePerUserGrowthPct =
    revenuePerUserAtPeriodStartCents > 0
      ...
      : last.revenuePerUserCents > 0
        ...
        : 0;
  const growthRatio =
    subscriberGrowthPct > 0 ? Math.round((revenueGrowthPct / subscriberGrowthPct) * 100) / 100 : null;
...
const buildTrendSeries = (orders: ReturnType<WabaBillingOrderRepository["list"]>) => {
...
export class WabaAdminDashboardService {
  constructor(
    private readonly financeiroService = new WabaAdminFinanceiroService(),
    private readonly subscribersService = new WabaAdminSubscribersService(),
    private readonly usersService = new WabaAdminUsersService(),
    private readonly campanhasService = new WabaOperacionalCampanhasService(),
  ...
  private resolveCapabilities(auth: AdminDashboardAuth) {
    const isMaster = auth.role === "master";
    const user = isMaster ? null : this.systemUserService.getByEmail(auth.email);
    const canCampanhas = isMaster || Boolean(user && isMenuAllowedForUser(user, "admin-campanhas"));
  ...
  private buildRecentActivity(
    orders: ReturnType<WabaBillingOrderRepository["list"]>,
    subscribers: ReturnType<WabaAdminSubscribersService["listSubscribers"]>,
    campaigns: ReturnType<WabaOperacionalCampanhasService["listCampaigns"]>,
    capabilities: {
      finance: boolean;
      subscribers: boolean;
      users: boolean;
    ...
    const events: Array<{
      at: string;
      type: "order" | "subscriber" | "campaign";
      label: string;
      detail: string;
      apiKind: string;
  ...
  async getOverview(auth: AdminDashboardAuth) {
    ...
    const revenueCents =
      Number(oficial?.contractedValueCents || 0) + Number(alternativa?.contractedValueCents || 0);
    const costCents =
      Number(oficial?.totalCostCents || 0) + Number(alternativa?.totalCostCents || 0);
    const profitCents =
    ...
    for (const settlement of settlements) {
      ...
      if (status === "pending" || status === "processing") splitPayoutPending += 1;
      if (status === "failed" || status === "partial") splitPayoutFailed += 1;
    }
    ...
    for (const user of users) {
      ...
      if (role in usersByRole) {
        usersByRole[role as keyof typeof usersByRole] += 1;
      } else {
        usersByRole.other += 1;
      }
    }
    ...
    for (const campaign of campaigns) {
      ...
      if (status === "completed") campaignsCompleted += 1;
      else campaignsOpen += 1;
      if (campaign.isStartOverdue) campaignsOverdue += 1;
      plannedSendCount += Number(campaign.plannedSendCount || 0);
      if (campaign.apiKind === "oficial") campaignsByApi.oficial += 1;
      if (campaign.apiKind === "alternativa") campaignsByApi.alternativa += 1;
    }
    ...
    return {
      ...
      integration: capabilities.finance
        ? {
            paymentConfigured: financeOverview?.config?.paymentConfigured ?? isAsaasConfigured(),
