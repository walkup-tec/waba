...
export type WhatsappConnectingRestrictionRow = {
  ...
  /** Origem: s├│ `explicit` gera tag. `connecting` autom├ítico foi desativado (falso positivo). */
...
export async function syncWhatsappConnectingRestriction(
  ...
  if (source !== "explicit") {
    ...
    console.warn(
      `[WA-Restri├º├úo] removida tag autom├ítica de ${name} (live=${state || "ÔÇö"}; connecting Ôëá restri├º├úo).`,
    );
    return null;
  }
...
export async function markWhatsappRestrictionExplicit(
  ...
  await saveStore(store);
  console.warn(
    `[WA-Restri├º├úo] expl├¡cita em ${name} at├® ${row.restrictedUntil}${detail ? ` (${String(detail).slice(0, 120)})` : ""}.`,
  );
  return { ...row };
}
