...
export function isPortalChallengeHint(input: {
  title?: string;
  url?: string;
  body?: string;
}): boolean {
  ...
  if (
    /__cf_chl|cf-challenge|cdn-cgi\/challenge|challenges\.cloudflare|turnstile|cf-browser-verification/i.test(
      url,
    )
  ) {
    return true;
...
export function isPortalAntiBotBlock(error: unknown): boolean {
  const msg = error instanceof Error ? error.message : String(error || "");
  return /ANTI_BOT|anti-bot|um momento|just a moment|cloudflare|turnstile|verifica[cç][aã]o de seguran|attention required|access denied|are you a robot|cf-challenge|__cf_chl|bloqueou o robô/i.test(
    msg,
  );
}
...
async function nudgeCloudflareChallenge(
  ...
  const frameSel =
    'iframe[src*="challenges.cloudflare"], iframe[src*="turnstile"], iframe[title*="Widget"]';
...
async function waitPastCloudflare(
  ...
  const timeoutMs = Math.max(
    8000,
    Math.round(
      Number(options?.timeoutMs ?? (Number(process.env.CASADOSDADOS_CF_WAIT_MS || 90000) || 90000)),
    ),
  );
  ...
  let lastPulse = 0;
  const pulse = (detail: string) => {
    const elapsed = Math.max(0, Math.round((Date.now() - started) / 1000));
    const now = Date.now();
    if (now - lastPulse < 7000 && lastPulse > 0) return;
    lastPulse = now;
    options?.onProgress?.(
      `Abrindo Portal: verificação anti-bot (${stage}) — ${detail} (${elapsed}s/${Math.round(timeoutMs / 1000)}s)…`,
    );
  };
  ...
  throw new LeadsScrapeError(
    "ANTI_BOT",
    "new-browser",
    `Portal Casa dos Dados ainda em verificação anti-bot (${stage}). ` +
...
async function gotoWithNodeBudget(
  ...
  const pwTimeout = Math.max(8_000, Math.min(35_000, budgetMs - 2_000));
  const ok = await withNodeTimeout(
    gotoWithRetry(page, url, { waitUntil: "domcontentloaded", timeout: pwTimeout }).then(
      () => true as const,
    ),
    budgetMs,
    false as const,
...
async function loginCasaDosDadosPortal(
  ...
  try {
    ...
    if (
      /locator\.waitFor|Timeout \d+ms exceeded|input\[name=["']email|input\[name=["']senha|Acessar continua disabled|ainda em \/entrar/i.test(
        msg,
      )
    ) {
...
function requiresBrowserRecovery(error: unknown): boolean {
  ...
  if (anyErr?.code === "ANTI_BOT") return true;
  if (isSoftScrapeError(error)) return false;
  if (isPortalAntiBotBlock(error)) return true;
...
function sleepNode(ms: number): Promise<void> {
 ...
 */
async function cdpOrReconnect<T>(
  promise: Promise<T>,
  timeoutMs: number,
  label: string,
): Promise<T> {
  type Race = { ok: true; value: T } | { ok: false };
  const result = await withNodeTimeout<Race>(
    promise.then((value): Race => ({ ok: true, value })),
    timeoutMs,
    { ok: false },
  );
  ...
  throw new LeadsScrapeError(
    "CDP_PROBE_TIMEOUT",
    "new-browser",
    `${label} não respondeu em ${Math.round(timeoutMs / 1000)}s (CDP/DOM travado) — reconectar Chromium.`,
  );
}
...
async function clickSearchDom(page: PageLike): Promise<boolean> {
  ...
  const candidates = await withNodeTimeout(
    findSearchButtonCandidatesFast(page),
    4000,
    null as SearchButtonCandidate[] | null,
  );
  ...
  return withNodeTimeout(
    page.evaluate((needle: string) => {
      ...
      for (const raw of Array.from(document.querySelectorAll(selector))) {
        ...
        if (
          (wantNorm && (text === wantNorm || text.includes(wantNorm))) ||
          text.includes("pesquisar") ||
          text.includes("buscar") ||
          aria.includes("pesquisar") ||
          aria.includes("buscar")
...
async function scrapeCasaDosDadosLeadsOnce(
  ...
  if (!browser.isConnected()) {
    await releaseJobBrowser(browser, "not-connected-at-start");
    browser = await acquireSharedBrowser({ headless, slowMo, hasXvfb });
  }
  ...
  try {
    ...
    if (!usedFastResume) {
      ...
      if (!wantFastResume) {
        ...
        await gotoWithNodeBudget(page, PORTAL_LOGIN_URL, "login", 40_000);
        ...
        await gotoWithNodeBudget(page, PORTAL_SEARCH_URL, "pesquisa-retomada", 40_000);
        await waitPastCloudflare(page, { onProgress: markPhase, stage: "pesquisa" });
      }
      ...
      if (!alreadyIn) {
        ...
        await gotoWithNodeBudget(page, PORTAL_SEARCH_URL, "pesquisa-pos-login", 40_000);
        await waitPastCloudflare(page, { onProgress: markPhase, stage: "pesquisa" });
      } else {
      ...
      if (!wantFastResume) {
        ...
        await gotoWithNodeBudget(page, PORTAL_SEARCH_URL, "pesquisa-filtros", 40_000);
        await waitPastCloudflare(page, { onProgress: markPhase, stage: "pesquisa" });
        await sleepNode(800);
      }
    ...
    const readCurrentPageNumber = async (): Promise<number> =>
      cdpOrReconnect(
        ...
        5000,
        "Leitura do número da página Oruga",
      );
    ...
    const jumpToPageDom = async (target: number): Promise<boolean> => {
      ...
      const attempt = await withNodeTimeout(
        page
          .evaluate((pageTarget: number) => {
        ...
        4000,
        { ok: false, how: "err" as const },
      );

      if (!attempt.ok) return false;
      return waitUntilPage(t, attempt.how === "input" ? 8_000 : 5_000);
    };

    /** Maior botão numérico visível com N > current e N <= target (aproxima em saltos). */
    const hopTowardPageDom = async (target: number): Promise<number> => {
      const t = Math.max(1, Math.round(target || 1));
      const hopped = await withNodeTimeout(
        page
          .evaluate((pageTarget: number) => {
            ...
            for (const b of Array.from(nav.querySelectorAll("button.pagination-link"))) {
              const text = String(b.textContent || "").trim();
              const n = Number(text);
              if (!Number.isFinite(n) || n <= current || n > pageTarget) continue;
      ...
      if (hopped <= 0) return 0;
      const moved = await waitUntilPage(hopped, 5_000);
    ...
    const goToNextResultsPage = async (
      ...
      const clicked = await withNodeTimeout(
        page
          .evaluate(() => {
            const nav = document.querySelector('nav[data-oruga="pagination"]');
            if (!nav) return false;
      ...
      while (Date.now() < deadline) {
        if (options?.shouldAbort?.()) return false;
        const cur = await withNodeTimeout(
          page
            .evaluate(() => {
              const active = document.querySelector(
                [
        ...
        if (previousFirstCnpj) {
          const nextFirst = await withNodeTimeout(
            readFirstVisibleCnpjDigits(page),
            1500,
            "",
          );
    ...
    const goToResultsPage = async (targetPage: number): Promise<boolean> => {
      ...
      const envBoost = Math.max(
        0,
        Math.round(Number(process.env.CASADOSDADOS_MAX_SEQUENTIAL_RESUME_STEPS || 0) || 0),
      );
      ...
      while (guard < maxSteps) {
        ...
        if (!ok) {
          ...
          if (stalled >= 3) {
            markPhase(
              `Copiando: paginação não avançou (UI ${current} → ${target}) — abortando posicionamento.`,
            );
            return false;
          }
          await sleepNode(400);
    ...
    if (portalTotal != null) {
      ...
      markPhase(
        `Portal: ${portalTotal.toLocaleString("pt-BR")} empresas · ${PORTAL_PAGE_SIZE}/página · ~${totalPagesAvailable.toLocaleString("pt-BR")} pág. no total · meta de cópia ${
          maxPagesCap > 0 ? pagesToFetch : "todas até o fim"
        }…`,
      );
