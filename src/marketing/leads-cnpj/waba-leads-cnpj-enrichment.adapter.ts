...
import { normalizeCnpjDigits } from "./waba-leads-cnpj.repository";
...
function mapReceitaPayload(payload: Record<string, unknown>, fallbackCnpj: string): WabaLeadsCnpjLead {
  const logradouro = pickString(payload.logradouro);
  const numero = pickString(payload.numero);
  return {
    cnpj: normalizeCnpjDigits(payload.cnpj || fallbackCnpj),
...
function receitaWsMinGapMs(): number {
...
let receitaWsTail: Promise<unknown> = Promise.resolve();

async function withReceitaWsPace<T>(fn: () => Promise<T>): Promise<T> {
  let release!: () => void;
  const gate = new Promise<void>((resolve) => {
    release = resolve;
  });
  const previous = receitaWsTail;
  receitaWsTail = previous.then(
    () => gate,
    () => gate,
  );
...
export async function enrichViaReceitaWs(cnpj: string, apiKey: string): Promise<WabaLeadsCnpjLead> {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      ...
      try {
        ...
        if (response.status === 429) {
          if (attempt >= maxAttempts) throw new Error("ReceitaWS HTTP 429");
          lastReceitaWsAt = Date.now();
...
async function enrichViaN8nWebhook(
  listId: string,
  leads: WabaLeadsCnpjLead[],
  webhookUrl: string,
): Promise<WabaLeadsCnpjLead[] | null> {
  const controller = new AbortController();
  ...
  try {
    ...
    return rows.map((row) => {
      ...
      return {
        cnpj: normalizeCnpjDigits(item.cnpj || item.CNPJ),
...
export type EnrichLeadsProgress = (message: string) => void;
export type EnrichLeadsCheckpoint = (
  snapshot: WabaLeadsCnpjLead[],
  doneCount: number,
) => void;

/** Heurística: lead já passou pela ReceitaWS (fecha lote parcial do dia). */
export function leadLooksEnriched(lead: WabaLeadsCnpjLead): boolean {
  if (lead.enriched) return true;
  if (String(lead.dataAbertura || "").trim()) return true;
  if (String(lead.email || "").trim()) return true;
  if (String(lead.telefone || "").trim()) return true;
  const sit = String(lead.situacao || "").trim();
  if (sit && /^(ativa|baixada|inapta|nula|suspensa|falha)/i.test(sit)) return true;
  return false;
}
...
export function formatReceitaWsLegend(done: number, total: number, suffix: string): string {
...
async function withEnrichLock<T>(fn: () => Promise<T>): Promise<T> {
  let release!: () => void;
  const gate = new Promise<void>((resolve) => {
    release = resolve;
  });
  const previous = enrichTail;
  enrichTail = previous.then(
    () => gate,
    () => gate,
  );
  await previous.catch(() => undefined);
...
export async function enrichLeadsCnpjList(
  listId: string,
  leads: WabaLeadsCnpjLead[],
  onProgress?: EnrichLeadsProgress,
  onCheckpoint?: EnrichLeadsCheckpoint,
  shouldAbort?: () => boolean,
  ...
  const pulse = setInterval(() => {
    ...
    onProgress?.(
      formatReceitaWsLegend(already, total, "Na fila (aguardando outra lista)…"),
    );
  }, 2000);
...
async function enrichLeadsCnpjListUnlocked(
  listId: string,
  leads: WabaLeadsCnpjLead[],
  onProgress?: EnrichLeadsProgress,
  onCheckpoint?: EnrichLeadsCheckpoint,
  ...
  if (webhookUrl) {
    ...
    try {
      ...
      if (error instanceof Error && error.message === "__MLC_JOB_ABORTED__") throw error;
  ...
  const out: WabaLeadsCnpjLead[] = leads.map((lead) => ({
    ...lead,
  ...
  for (let index = 0; index < out.length; index += 1) {
    ...
    if (hasPending) {
      await sleepWithCountdown(delayMs, (leftMs) => {
        if (shouldAbort?.()) throw new Error("__MLC_JOB_ABORTED__");
        onProgress?.(
          formatReceitaWsLegend(done, total, `Próximo em ${formatMmSs(leftMs)}`),
        );
      });
    } else {
