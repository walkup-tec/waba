 ...
 */
const INBOUND_PRESTART_GRACE_MS = Math.max(
  30_000,
  Math.min(
    600_000,
    Number(process.env.INBOUND_VALIDATION_PRESTART_GRACE_MS || 180_000) || 180_000,
  ),
);

/** @internal exported for unit tests */
export function inboundAcceptMinTimestampMs(record: {
  validationStartedAtMs: number;
  keywordHighWaterMarkMs: number;
}): number {
...
async function pollReceiveIfDue(record: ValidationRecord): Promise<void> {
  ...
  try {
    ...
    if (hit) markInboundReceived(record, hit, deep ? "resolve-deep" : "resolve-fast");
  } catch {
    record.receivePollCache = {
      atMs: now,
      hit: null,
...
export async function startInboundValidation(input: {
  ...
  console.info(
    ...
    `minTs=${new Date(inboundAcceptMinTimestampMs({ validationStartedAtMs, keywordHighWaterMarkMs })).toISOString()}`,
    `captured=${capturedWaterMarkMs ? new Date(capturedWaterMarkMs).toISOString() : "0"}`,
    `graceMs=${INBOUND_PRESTART_GRACE_MS}`,
  );
