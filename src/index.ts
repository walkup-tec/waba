...
function ensureMessageContainsLink(message: string, link: string, cta: string) {
  ...
  if (text.includes(safeLink)) return text;
...
app.get("/disparos/next-instance", async (_req, res) => {
  try {
    ...
    const selectedSet = new Set(
      Array.isArray(disparosConfig.selectedDisparadorInstances)
...
async function composeOutboundMessageForConfig(config: DisparosConfig): Promise<string> {
