import type { VpsCpuPlaybook } from "./vps-cpu-monitor.types";

const traefikAccessLogPlaybook: VpsCpuPlaybook = {
  ...
  steps: [
    {
      ...
      command:
        "docker service inspect easypanel-traefik --format '{{range .Spec.TaskTemplate.ContainerSpec.Env}}{{println .}}{{end}}' | grep -iE 'TRAEFIK_(ACCESSLOG|API)'",
    },
    {
      order: 2,
      title: "Desligar access log e dashboard (snapshot hPanel antes)",
      command:
        "docker service update --env-rm TRAEFIK_ACCESSLOG TRAEFIK_API_DASHBOARD TRAEFIK_API_INSECURE --env-add TRAEFIK_ACCESSLOG=false --env-add TRAEFIK_API_DASHBOARD=false --env-add TRAEFIK_API_INSECURE=false easypanel-traefik",
    },
    {
      order: 3,
    ...
    },
    {
      ...
      command:
        'docker stats --no-stream --format "{{.Name}} {{.CPUPerc}}" | grep -i traefik',
    },
  ],
};

const typebotPlaybook: VpsCpuPlaybook = {
  id: "typebot-stack",
  ...
  steps: [
    ...
    },
    {
      ...
      command:
        "docker service ls --format '{{.Name}}' | grep -i '^typebot_' | xargs -r docker service rm",
...
const easypanelPlaybook: VpsCpuPlaybook = {
  id: "easypanel-panel",
...
const n8nPlaybook: VpsCpuPlaybook = {
  id: "n8n",
...
const chatwootPlaybook: VpsCpuPlaybook = {
  id: "chatwoot",
  ...
  steps: [
    ...
    },
    {
      ...
      title: "Remover stack",
      command:
        "docker service rm walkup_chatwoot-sidekiq walkup_chatwoot-walkup walkup_chatwoot-redis-walkup walkup_chatwoot-db-walkup",
    },
    {
      order: 3,
      title: "Validar WABA",
...
const dockerDaemonPlaybook: VpsCpuPlaybook = {
  id: "docker-daemon",
  label: "Docker daemon",
  ...
  steps: [
    ...
    },
    {
      ...
      command:
        "docker service ls | grep -E 'traefik|waba_disparador' && curl -sS http://127.0.0.1:30180/health | head -c 120",
    },
  ],
};
...
const genericPlaybook: VpsCpuPlaybook = {
  id: "generic",
  ...
  steps: [
    {
      ...
      command:
        "docker stats --no-stream --format '{{.Name}}\\t{{.CPUPerc}}\\t{{.MemUsage}}' | sort -t$'\\t' -k2 -hr | head -15",
    },
    {
      order: 2,
...
export const resolveServiceKeyFromContainerName = (name: string): string => {
  const n = String(name || "").toLowerCase();
  if (n.includes("easypanel-traefik") || n.includes("traefik")) return "traefik";
  if (n.includes("chatwoot") || n.includes("sidekiq") || n.includes("puma")) return "chatwoot";
  if (n.includes("n8n")) return "n8n";
  if (n.includes("typebot")) return "typebot";
  if (n.includes("easypanel.") && !n.includes("traefik")) return "easypanel";
  if (n.includes("waba_disparador") || n.includes("waba-disparador")) return "waba";
  if (n.includes("evo-walkup") || n.includes("walkup-api")) return "evolution";
  if (n.includes("dockerd") || n === "dockerd") return "docker";
  return "generic";
};

export const resolvePlaybookForServiceKey = (serviceKey: string): VpsCpuPlaybook => {
  switch (serviceKey) {
    case "traefik":
